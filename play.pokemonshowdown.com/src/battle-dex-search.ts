			...
			if (topbufIndex < 0 && searchTypeIndex < 2 && passType === 'alias' && !bufs[1].length && bufs[2].length) {
				topbufIndex = 2;
						...
						if (
							...
							(!table || !table.overrideItemInfo || !(id in table.overrideItemInfo))
						) continue;
						else if (
							typeIndex === 4 && (!BattleMovedex[id] || BattleMovedex[id].exists === false) &&
							(!table || !table.overrideMoveInfo || !(id in table.overrideMoveInfo))
						) continue;
						else if (
							typeIndex === 6 && (!BattleAbilities[id] || BattleAbilities[id].exists === false) &&
							(!table || !table.overrideAbilityDesc || !(id in table.overrideAbilityDesc))
						) continue;
						else if (
							typeIndex === 2 && !(id.replace(id.charAt(0), id.charAt(0).toUpperCase()) in window.BattleTypeChart) && (!table || !(id.replace(id.charAt(0), id.charAt(0).toUpperCase()) in table.overrideTypeChart))
						) continue;
...
class BattleItemSearch extends BattleTypedSearch<'item'> {
	...
	getBaseResults(): SearchRow[] {
		...
		for (let i = results.length - 1; i > 0; i--) {
			...
			if(item.name === 'boosterenergy' && species.tags?.includes('Paradox')) {
