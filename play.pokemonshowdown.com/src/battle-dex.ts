...
export const Dex = new class implements ModdedDex {
	...
	getTeambuilderSpriteData(pokemon: any, dex: ModdedDex = Dex, mod: string = ''): TeambuilderSpriteData {
		...
		mod = this.getSpriteMod(mod, id, 'front', species.exists);
		if (mod) {
			return {
	...
	getTeambuilderSprite(pokemon: any, dex?: ModdedDex, xOffset = 0, yOffset = 0, mod: string = '') {
		if (!pokemon) return '';
		const data = this.getTeambuilderSpriteData(pokemon, dex, mod);
