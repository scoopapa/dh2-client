...
const Moves = {
  gmaxsteelsurge: {
    ...
    condition: {
      ...
      onSwitchIn(pokemon) {
        ...
        if (pokemon.hasAbility("trashcompactor") || pokemon.hasItem("heavydutyboots"))
          return;
  ...
  sparksplosion: {
    ...
    shortDesc: "Hits, paralyzes adjacent Pok\xE9mon. The user faints.",
    name: "Sparksplosion",
    pp: 5,
    priority: 0,
  ...
  inverseroom: {
    ...
    basePower: 0,
    category: "Status",
    desc: "For 5 turns, all Pok\xE9mon on the field are resistant to normally super-effective types and weak to normally not-very-effective types (as in Inverse Battles). Immunities are not bypassed.",
  ...
  guillotine: {
    ...
    basePower: 0,
    category: "Physical",
    desc: "Deals damage to the target equal to the target's maximum HP. Ignores accuracy and evasiveness modifiers. This attack's accuracy is equal to (user's level - target's level + 30)%, and fails if the target is at a higher level. Pok\xE9mon with the Sturdy Ability are immune. For Escavalier from Sylvemons, the move's base power is 130, its accuracy is 100, and it lowers the user's Attack by 2 stages.",
  ...
  solarbeam: {
    ...
    onTryMove(attacker, defender, move) {
      if (attacker.species.id === "claydol")
        return;
