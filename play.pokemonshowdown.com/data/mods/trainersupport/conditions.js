...
const Conditions = {
  larryboost: {
    ...
    onBasePower(basePower, pokemon, target, move) {
      if (move.type === "Normal" || move.type === "Flying")
  ...
  roxanneboost: {
    ...
    onSourceModifyAccuracy(accuracy, target, source, move) {
      if (typeof accuracy !== "number" || move.type !== "Rock")
        return;
      return this.chainModify([4915, 4096]);
    }
  },
  sidneyboost: {
    ...
    onTryBoost(boost, target, source, effect) {
      if (!source || target === source || !boost || effect.name === "Mirror Armor" || effect.name === "sidneyboost")
        return;
      let b;
      for (b in boost) {
        if (boost[b] < 0) {
  ...
  laceyboost: {
    ...
    noCopy: true,
    onModifyMove(move) {
      if (move.type === "Fairy" || move.type === "Ground")
      ...
      if (!source.hasType("Psychic") && !source.terastallized) {
        ...
        for (const moveSlot of source.moveSlots) {
          if (moveSlot.id === "futuresight") {
            if (source.addType("Psychic"))
              this.add("-start", source, "typeadd", "Psychic");
          }
        }
      }
    }
  ...
  cguysboost: {
    ...
    onBasePowerPriority: 23,
    onBasePower(basePower, pokemon, target, move) {
      if (["Grass", "Fire", "Water"].includes(move.type))
        return this.chainModify([4505, 4096]);
    }
  },
  lysandreboost: {
    ...
    onAnyBasePower(basePower, source, target, move) {
      if (target === source || move.category === "Status" || target.getMoveHitData(move).typeMod < 0)
        return;
      if (!move.auraBooster?.volatiles["lysandreboost"])
  ...
  whitneyboost: {
    ...
    onTryBoost(boost, target, source, effect) {
      if (source && (target === source || target.gender !== "F"))
        return;
      let showMsg = false;
      let i;
      for (i in boost) {
  ...
  rileyboost: {
    ...
    onBasePowerPriority: 23,
    onBasePower(basePower, pokemon, target, move) {
      if (move.type === "Fighting" && pokemon.hp < pokemon.baseMaxhp / 2)
  ...
  sandstorm: {
    ...
    durationCallback(source, effect) {
      ...
      if (source.side.trainerBoost === "steven")
        duration += 2;
      return duration;
    }
  }
