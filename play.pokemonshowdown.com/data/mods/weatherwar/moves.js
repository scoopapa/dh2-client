...
const Moves = {
    ...
    condition: {
      ...
      onAnyPrepareHit(source, target, move) {
        ...
        if (snatchUser.isSkyDropped())
          return;
        if (this.field.pseudoWeather.twilightzone && (!move || move.basePower > 60))
          return;
  ...
  firepledge: {
    ...
    onModifyMove(move) {
      if (this.field.pseudoWeather.drought)
  ...
  destinybond: {
    ...
    onPrepareHit(pokemon) {
      if (this.field.pseudoWeather.thevoices)
  ...
  grasspledge: {
    ...
    onModifyMove(move) {
      if (this.field.pseudoWeather.overgrowth)
  ...
  sandtomb: {
    ...
    onModifyMove(move) {
      if (this.field.pseudoWeather.duststorm)
        move.volatileStatus = "sandtomb";
    },
    condition: {
      noCopy: true,
      onStart(pokemon) {
      ...
      onResidual(pokemon) {
        if (this.field.pseudoWeather.duststorm)
          this.boost({ def: -1 }, pokemon, pokemon, this.dex.getActiveMove("sandtomb"));
      },
      onEnd(pokemon) {
  ...
  iceball: {
    ...
    condition: {
      duration: 1,
      onLockMove(pokemon) {
        if (this.field.getPseudoWeather("whiteout"))
          return;
        return "iceball";
      },
      onStart() {
  ...
  metronome: {
    ...
    onHit(target, source, effect) {
      ...
      const moves = this.dex.moves.all().filter((move) => this.field.pseudoWeather.metronomebattle ? move.basePower >= 75 && !move.flags["charge"] || GOOD_STATUS_MOVES.includes(move) || GOOD_WEAK_MOVES.includes(move) : (
        //any viable move
        (![2, 4].includes(this.gen) || !source.moves.includes(move.id)) && (!move.isNonstandard || move.isNonstandard === "Unobtainable") && move.flags["metronome"]
      ));
      let randomMove = "";
      if (moves.length) {
  ...
  belch: {
    ...
    onDisableMove(pokemon) {
      if (!this.field.pseudoWeather.shitstorm && !pokemon.ateBerry)
  ...
  metalclaw: {
    ...
    onModifyMove(move) {
      if (this.field.pseudoWeather.timewarp) {
        move.onAfterHit = function(target, source, move2) {
  ...
  waterpledge: {
    ...
    onModifyMove(move) {
      if (this.field.pseudoWeather.flashflood)
          ...
          for (const target of this.sides[1].active) {
            if (target.storedStats.spe < pokemon.storedStats.spe) {
              ...
              if (oldAbility) {
                ...
                if (target.side !== pokemon.side)
                  pokemon.volatileStaleness = "external";
                return;
              }
            } else {
  ...
  stickyweb: {
    ...
    condition: {
      ...
      onEntryHazard(pokemon) {
        if (!pokemon.isGrounded() || pokemon.hasItem("heavydutyboots") || pokemon.hasAbility("sandrush"))
          return;
