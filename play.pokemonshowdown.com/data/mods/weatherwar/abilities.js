...
const Abilities = {
  ...
  itsspring: {
    onStart(source) {
      ...
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${source.name}|It's spring!`);
      this.field.addPseudoWeather("overgrowth");
      this.field.addPseudoWeather("fable");
    },
    flags: {},
  ...
  ambush: {
    ...
    onModifyMove(move, source, target) {
      move.accuracy = true;
      if (this.field.pseudoWeather.twilightzone && (target?.beingCalledBack || target?.switchFlag))
        move.accuracy = true;
    },
  ...
  banshee: {
    ...
    onModifyType(move, pokemon) {
      ...
      if (this.field.pseudoWeather.thevoices && move.type === "Normal" && !noModifyType.includes(move.id) && !(move.isZ && move.category !== "Status") && !(move.name === "Tera Blast" && pokemon.terastallized)) {
        move.type = "Ghost";
  ...
  sandforce: {
    ...
    onResidualSubOrder: 2,
    onResidual(pokemon) {
      if (!this.field.pseudoWeather.duststorm || !pokemon.hp)
        return;
  ...
  duomodreference: {
    onResidual(pokemon) {
      if (!this.field.pseudoWeather.metronomebattle)
        return;
      ...
      if (result === 0) {
        ...
        for (const pokemon2 of this.getAllActive()) {
          this.heal(pokemon2.maxhp, pokemon2);
          pokemon2.cureStatus();
        }
      } else if (result === 1) {
        ...
        for (const pokemon2 of this.getAllActive()) {
          ...
          let bestStat = 0;
          let s;
          for (s in pokemon2.storedStats) {
            if (pokemon2.storedStats[s] > bestStat) {
              statName = s;
              bestStat = pokemon2.storedStats[s];
            }
          }
          this.boost({ [statName]: 3 }, pokemon2);
        }
      } else if (result === 2) {
        ...
        for (const pokemon2 of this.getAllActive()) {
          this.actions.useMove("Spikes", pokemon2);
        }
      } else if (result === 6) {
        ...
        for (const pokemon2 of this.getAllActive()) {
          this.actions.useMove("Conversion 2", pokemon2);
        }
      } else if (result === 11) {
        ...
        for (const pokemon2 of this.getAllActive()) {
          this.actions.useMove("Camouflage", pokemon2);
        }
      } else if (result === 19) {
        ...
        for (const pokemon2 of this.getAllActive()) {
          this.actions.useMove("Safeguard", pokemon2);
        }
      } else if (result === 30) {
        ...
        for (const pokemon2 of this.getAllActive()) {
          this.actions.useMove("Aqua Ring", pokemon2);
        }
      } else if (result === 34) {
        ...
        for (const pokemon2 of this.getAllActive()) {
          this.actions.useMove("Sheer Cold", pokemon2);
        }
      } else if (result === 37) {
        this.hint("Roulette Wheel Result 38 - uh oh");
        for (const pokemon2 of this.getAllActive()) {
          this.actions.useMove("Octolock", pokemon2);
        }
      } else if (result === 38) {
        this.hint("Roulette Wheel Result 39 - Both active Pokemon use Metronome.");
        for (const pokemon2 of this.getAllActive()) {
          this.actions.useMove("Metronome", pokemon2);
        }
      } else if (result === 39) {
        ...
        for (const pokemon2 of this.getAllActive()) {
          const frstMove = this.dex.moves.get(pokemon2.moveSlots[0].id);
          this.actions.useMove(frstMove, pokemon2);
        }
      } else if (result === 43) {
        ...
        for (const pokemon2 of this.getAllActive()) {
          this.actions.useMove("Roulette Spin", pokemon2);
        }
      } else if (result === 45) {
        ...
        for (const pokemon2 of this.sides[0].active) {
          for (const target of this.sides[1].active) {
            if (target.storedStats.spe < pokemon2.storedStats.spe) {
              this.actions.useMove("Flame Runner", pokemon2);
              const oldAbility = target.setAbility("Slow Start");
              if (oldAbility) {
                this.add("-ability", target, "Slow Start", "[from] move: Flame Runner", "[silent]");
                if (target.side !== pokemon2.side)
                  target.volatileStaleness = "external";
                return;
              }
            } else if (target.storedStats.spe > pokemon2.storedStats.spe) {
              this.actions.useMove("Flame Runner", target);
              const oldAbility = pokemon2.setAbility("Slow Start");
              if (oldAbility) {
                this.add("-ability", pokemon2, "Slow Start", "[from] move: Flame Runner", "[silent]");
                if (target.side !== pokemon2.side)
                  pokemon2.volatileStaleness = "external";
                return;
              }
            } else {
        ...
        for (const pokemon2 of this.getAllActive()) {
          this.actions.useMove("Ultranome", pokemon2);
        }
      }
      if (pokemon.metronomeUsed)
        delete pokemon.metronomeUsed;
    },
  ...
  corrosion: {
    inherit: true,
    onModifyMove(move) {
      if (!this.field.pseudoWeather.shitstorm)
        return;
    ...
    onEffectiveness(typeMod, target, type, move) {
      if (this.field.pseudoWeather.shitstorm && move.type === "Poison" && type === "Steel")
        return 1;
    },
  ...
  domainexpander: {
    ...
    onBasePowerPriority: 23,
    onBasePower(basePower, pokemon, target, move) {
      if (this.field.pseudoWeather.mindfuck && typeof accuracy === "number" && move.accuracy === 100)
  ...
  wetskin: {
    ...
    onResidualSubOrder: 2,
    onResidual(pokemon) {
      if (this.field.pseudoWeather.wetskin || !pokemon.hp)
        return;
  ...
  cloudnine: {
    onStart(pokemon) {
      ...
      for (let i = 0; i < 2; i++) {
        if (this.field.pseudoWeather.length == 0)
          break;
  ...
  forecast: {
    ...
    onAnyPseudoWeatherChange(target, source, pseudoWeather) {
      ...
      if (pokemon.setType(newType))
        this.add("-start", pokemon, "typechange", newType.join("/"));
    },
