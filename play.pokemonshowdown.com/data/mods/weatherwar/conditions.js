...
const Conditions = {
  ...
  theswarm: {
    ...
    durationCallback(source, effect) {
      if (source?.hasItem("therock"))
        return 8;
      if (source.hasAbility("weathersetter"))
        return 0;
      return 5;
    },
    ...
    onModifyMove(move) {
      if (move.category === "Status" || move.type !== "Bug")
        return;
  ...
  twilightzone: {
    ...
    onModifyAccuracy(accuracy, target, source, move) {
      if (!this.field.pseudoWeather.twilightzone || typeof accuracy !== "number")
        return;
      if (move.type !== "Dark") {
    ...
    onModifyMove(move, pokemon) {
      if (this.field.pseudoWeather.twilight && move.basePower <= 60)
        move.flags["snatch"] = 1;
    },
  ...
  lotsofreallysmalldragons: {
    ...
    onPrepareHit(source, target, move) {
      if (!this.field.pseudoWeather.lotsofreallysmalldragons || move.category === "Status" || move.type !== "Dragon" || move.flags["noparentalbond"] || move.flags["charge"] || move.flags["futuremove"] || move.spreadHit || move.isZ || move.isMax)
        return;
      if (move.multihit)
        move.multihit *= 2;
      else
  ...
  thunderstorm: {
    ...
    onResidual(pokemon) {
      if (this.field.pseudoWeather.thunderstorm && pokemon.hp && this.randomChance(3, 10)) {
        ...
        const newMon = new import_pokemon.Pokemon(newSet, pokemon.side);
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|LTG|You should forfeit... NOW!`);
        this.add("-anim", pokemon, "Thunder", pokemon);
        this.actions.useMove(newMove, newMon, pokemon);
      }
    },
    onModifyMove(move, pokemon) {
      if (this.field.pseudoWeather.thunderstorm && move.type === "Electric")
        move.accuracy = true;
    },
    onFieldStart(field, source, effect) {
      if (effect?.effectType === "Ability") {
  ...
  fable: {
    ...
    onSourceModifyAtk(atk, attacker, defender, move) {
      if (this.field.pseudoWeather.fable && ["Dark", "Dragon", "Ghost", "Poison"].includes(move.type)) {
        if (!attacker.hasAbility("darkfantasy"))
  ...
  thevoices: {
    ...
    onModifyMove(move, pokemon) {
      if (!this.field.pseudoWeather.thevoices || move.type !== "Ghost")
        return;
  ...
  overgrowth: {
    ...
    onResidual(pokemon) {
      ...
      if (this.field.pseudoWeather.overgrowth && !pokemon.hasType("Grass"))
  ...
  whiteout: {
    ...
    onSourceDamagingHit(damage, target, source, move) {
      if (!this.field.pseudoWeather.whiteout || target.hasAbility("shielddust") || target.hasItem("covertcloak"))
        return;
      if (move.type === "Ice") {
  ...
  metronomebattle: {
    ...
    onResidual(pokemon) {
      if (!this.field.pseudoWeather.metronomebattle || !pokemon.hasType("Normal"))
        return;
      const dance = this.dex.getActiveMove("metronome");
      this.actions.useMove(dance, pokemon);
    },
    onEffectiveness(typeMod, target, type, move) {
      if (this.field.pseudoWeather.metronomebattle && move.type === "Normal")
        return 0;
    },
  ...
  shitstorm: {
    ...
    onResidual(pokemon) {
      if (!this.field.pseudoWeather.shitstorm || !pokemon.hp)
        return;
  ...
  mindfuck: {
    ...
    onModifyAccuracy(accuracy) {
      if (!this.field.pseudoWeather.mindfuck || typeof accuracy !== "number")
        return;
      return this.chainModify([6840, 4096]);
    },
    onDisableMove(pokemon) {
      if (!this.field.pseudoWeather.mindfuck)
        return;
  ...
  landslide: {
    ...
    onBasePower(basePower, pokemon, target, move) {
      if (!this.field.pseudoWeather.landslide || move.type !== "Rock" || typeof move.accuracy !== "number")
        return basePower;
        ...
        if (move.target === "self" && !pokemon.side.slotConditions[pokemon.position]["selfforesighter"]) {
          move.onTry = function(source, t) {
            ...
            if (source.hasAbility("secondimpact")) {
              if (!t.side.addSlotCondition(t, "orbitalfuturemove"))
                return false;
  ...
  flashflood: {
    ...
    onModifyPriority(priority, pokemon, target, move) {
      if (!this.field.pseudoWeather.flashflood || move?.category === "Status" || move?.type !== "Water")
        return;
