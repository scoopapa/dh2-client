...
const Conditions = {
  congestionstatus: {
    ...
    onStart(target) {
      this.effectState.congestionQueue = [];
    },
    ...
    onUpdate(pokemon) {
      const congestionStillActive = this.getAllActive().some(
        (p) => p.getAbility().id === "congestion" && !p.fainted
      );
      if (congestionStillActive)
        return;
      const slot = pokemon.position;
      const side = pokemon.side;
      const slotCon = side.slotConditions[slot];
      if (!slotCon?.congestionQueue?.length)
        return;
      this.add("-message", `${pokemon.name}'s slot resolves delayed status moves!`);
      for (const { move, sourceSlot, side: moveSide } of slotCon.congestionQueue) {
        ...
        const targets = this.getAllActive().filter((t) => {
          ...
          switch (moveTargetType) {
            case "self":
              return t === source;
            case "adjacentAlly":
              return t !== source && t.side === source.side && source.isAdjacent(t);
            case "adjacentAllyOrSelf":
              return t.side === source.side && source.isAdjacent(t);
            case "allySide":
              return t === source;
            case "foeSide":
              return t.side !== source.side;
            case "allAdjacentFoes":
              return t.side !== source.side && source.isAdjacent(t);
            case "allAdjacent":
              return source.isAdjacent(t);
            case "allyTeam":
              return t.side === source.side;
            case "all":
              return true;
            case "normal":
              return t !== source && t.side !== source.side;
            default:
              return false;
          }
        });
        for (const target of targets) {
          if (activeMove.status)
            target.trySetStatus(activeMove.status, source, activeMove);
          if (activeMove.volatileStatus)
            target.addVolatile(activeMove.volatileStatus, source, activeMove);
          if (activeMove.boosts)
            this.boost(activeMove.boosts, target, source, activeMove);
          if (activeMove.heal) {
