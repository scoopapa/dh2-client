...
const Abilities = {
  ...
  vengeful: {
    ...
    onBasePower(basePower, attacker2, defender, move) {
      if (attacker2.moveLastTurnResult === false) {
  ...
  shieldsdown: {
    inherit: true,
    onStart(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Minior" && !attacker.species.name.startsWith("Minior-Mega") || pokemon.transformed)
        return;
      if (pokemon.hp > pokemon.maxhp / 2) {
    ...
    onResidualOrder: 29,
    onResidual(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Minior" && !attacker.species.name.startsWith("Minior-Mega") || pokemon.transformed || !pokemon.hp)
        return;
      if (pokemon.hp > pokemon.maxhp / 2) {
        if (attacker.species.name.startsWith("Minior-Mega") && pokemon.species.forme !== "Mega-Meteor") {
          pokemon.formeChange("Minior-Mega-Meteor");
