...
const Abilities = {
  ...
  legendarybeast: {
    ...
    onAnyModifyAtk(atk, source, target, move) {
      ...
      if (source.hasAbility("Legendary Beast"))
        return;
      if (!move.ruinedAtk)
  ...
  lionspride: {
    ...
    onHit(target, source, move) {
      if (!source.isAlly(target))
        this.hint(move.category + " " + move.name);
    },
  ...
  shellbreaker: {
    onPrepareHit(source, target, move) {
      if (this.effectState.shellBreaker)
        return;
