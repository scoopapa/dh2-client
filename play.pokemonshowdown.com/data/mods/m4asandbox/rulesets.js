...
const Rulesets = {
  ...
  sandboxmod: {
    effectType: "Rule",
    name: "Sandbox Mod",
    desc: "Allows customization of a Pok\xE9mon's types and stats based on its nickname.",
    onBegin() {
      for (const pokemon of this.getAllPokemon()) {
        pokemon.m.originalSpecies = pokemon.species.name;
        if (!pokemon.set.name)
        ...
        if (pokemon.set.name.substr(0, 1) === "*") {
          ...
          if (["Mega Stone 1", "Mega Stone 2", "Mega Stone H"].includes(pokemon.getItem().name)) {
            ...
            if (pokemon.getItem().name === "Mega Stone 2" && abilities[1])
              ability = abilities[1];
            if (pokemon.getItem().name === "Mega Stone H" && abilities["H"])
              ability = abilities["H"];
