...
const Abilities = {
  ...
  gamble: {
    shortDesc: "This Pok\xE9mon's Metronome hits five times.",
  ...
  secretweapon: {
    shortDesc: "When this Pok\xE9mon faints, it restores its teammates' HP, PP and status.",
    onStart(pokemon) {
      ...
      this.hint("Pok\xE9mon that have already fainted won't be revived.");
    },
    onFaint(pokemon) {
      this.add("-ability", pokemon, "Secret Weapon");
      this.add("-message", `${pokemon.name} restored the HP, PP and status of its entire team!`);
      for (const ally of pokemon.side.pokemon) {
        if (!ally.fainted && (ally.hp < ally.maxhp || ally.status || ally.moveSlots.some((moveSlot) => moveSlot.pp < moveSlot.maxpp))) {
  ...
  amalgam: {
    shortDesc: "Eats the Steel type, removing it from other Pok\xE9mon to restore HP by 1/3.",
    desc: "This Pok\xE9mon eats the Steel type! At every possible opportunity, it removes the Steel type from adjacent Pok\xE9mon. Each time it does so, its own HP is restored by 1/3.",
    onUpdate(pokemon) {
  ...
  zenmode: {
    // altered onEnd so a Darmanitan-Zen can Mega Evolve properly
    onResidualOrder: 27,
    onResidual(pokemon) {
  ...
  flowergift: {
    // removed form dependence and prevented Mega Cherrim from changing form
    desc: "If this Pok\xE9mon is a Cherrim and Sunny Day is active, it changes to Sunshine Form and the Attack and Special Defense of it and its allies are multiplied by 1.5. If this Pok\xE9mon is a Mega Meganium and Sunny Day is active, the Attack and Special Defense of it and its allies are multiplied by 1.5. If this Pok\xE9mon is a Cherrim or a Mega Meganium and it is holding Utility Umbrella, it remains in its regular form and the Attack and Special Defense stats of it and its allies are not boosted. If this Pok\xE9mon is a Cherrim in its Sunshine form and is given Utility Umbrella, it will immediately switch back to its regular form. If this Pok\xE9mon is a Cherrim holding Utility Umbrella and its item is removed while Sunny Day is active, it will transform into its Sunshine Form. If an ally is holding Utility Umbrella while Cherrim is in its Sunshine Form or Meganium is Mega Evolved, they will not receive the Attack and Special Defense boosts.",
    ...
    onUpdate(pokemon) {
      if (!pokemon.isActive || pokemon.baseSpecies.baseSpecies !== "Cherrim" || pokemon.species.isMega || pokemon.transformed)
        return;
    ...
    onAllyModifyAtk(atk, pokemon) {
      if (this.effectState.target.baseSpecies.baseSpecies !== "Cherrim" && !this.effectState.target.species.isMega)
        return;
      if (["sunnyday", "desolateland"].includes(pokemon.effectiveWeather())) {
  ...
  stancechange: {
    // added compatibility for non-Falinks Megas (including Aegislash)
    desc: "If this Pok\xE9mon is an Aegislash or holding Falinksite, it changes to Blade Forme or Mega Combat before attempting to use an attacking move, and changes to Shield Forme or Mega Legion before attempting to use King's Shield.",
    shortDesc: "Changes to Blade Forme/Combat before attack, Shield Forme/Legion before King's Shield.",
    onBeforeMovePriority: 0.5,
    onBeforeMove(attacker, defender, move) {
      if (attacker.species.baseSpecies !== "Aegislash" && !attacker.species.isMega || attacker.transformed)
        return;
      ...
      if (attacker.species.baseSpecies === "Aegislash" && !attacker.species.isMega) {
        ...
        if (attacker.species.falinks !== targetForme) {
          ...
          const species = this.getMixedSpecies(baseSpecies, targetForme);
  ...
  sos: {
    // added compatibility for non-Wishiwashi Megas
    ...
    onResidualOrder: 27,
    onResidual(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Wishiwashi" && !pokemon.species.isMega || pokemon.transformed || !pokemon.hp || !pokemon.activeTurns || pokemon.species.id === "wishiwashi" || pokemon.species.id === "wishiwashischool" || pokemon.species.wishiwashi === "School")
        return;
      this.add("-activate", pokemon, "ability: SOS");
      this.add("-message", `${pokemon.name} called for help!`);
      if (!pokemon.species.wishiwashi) {
        const species2 = this.getMixedSpecies(pokemon.m.originalSpecies, "Wishiwashi-Mega-1");
        species2.wishiwashi = 1;
        pokemon.formeChange(species2, this.effect, true);
      } else if (pokemon.species.wishiwashi === 1) {
        const species2 = this.getMixedSpecies(pokemon.m.originalSpecies, "Wishiwashi-Mega-2");
        species2.wishiwashi = 2;
        pokemon.formeChange(species2, this.effect, true);
      } else if (pokemon.species.wishiwashi === 2) {
        const species2 = this.getMixedSpecies(pokemon.m.originalSpecies, "Wishiwashi-Mega-3");
        species2.wishiwashi = 3;
        pokemon.formeChange(species2, this.effect, true);
      } else if (pokemon.species.wishiwashi === 3) {
        const species2 = this.getMixedSpecies(pokemon.m.originalSpecies, "Wishiwashi-Mega-4");
        species2.wishiwashi = 4;
        pokemon.formeChange(species2, this.effect, true);
      } else if (pokemon.species.wishiwashi === 4) {
        const species2 = this.getMixedSpecies(pokemon.m.originalSpecies, "Wishiwashi-Mega-School");
        species2.wishiwashi = "School";
        pokemon.formeChange(species2, this.effect, true);
      }
  ...
  forgery: {
    ...
    onStart(pokemon) {
      if (!pokemon.species.isMega || pokemon.item !== "zoroarkite")
        return;
  ...
  contaminate: {
    shortDesc: "This Pok\xE9mon's Normal-type moves become Poison-type and have 1.2x power.",
  ...
  defibrillator: {
    desc: "When the Pok\xE9mon uses an Electric-type move, if any members of its party have status conditions, they are cured and affected teammates restore 1/6th of their maximum HP.",
    shortDesc: "Electric moves cure ally status and heal cured allies",
    onAfterMove(source, target, move) {
      if (move.type === "Electric") {
        for (const ally of source.side.pokemon) {
          if (ally.cureStatus())
            ally.heal(ally.baseMaxhp / 6);
        }
      }
    },
    name: "Defibrillator",
  ...
  modify: {
    desc: "As soon as this Pok\xE9mon Mega Evolves/switches in, it gains a random type. All Normal-type moves of this Pok\xE9mon become that type and gain a 1.25x boost.",
  ...
  sacrificialbarrier: {
    // wh
    desc: "As soon as this Pok\xE9mon uses a move or deals damage (in the case of Volt Switch), the Pok\xE9mon sets up Reflect and Light Screen, and then explodes. These Reflect and Light Screens last 15 turns.",
  ...
  omniscientsentinel: {
    desc: "The Pok\xE9mon's type becomes the type of its first two moveslots. Multi-Attack changes type to match the Pok\xE9mon's primary type.",
  ...
  rusty: {
    desc: "For every turn that the Pok\xE9mon is on the field, its Speed, Defense and Special Defense fall one stage. Becomes two stages if hit with a Water-type move.",
  ...
  afterburner: {
    desc: "When this Pok\xE9mon uses a Fire-type move, or is hit by a damaging Fire-type move, its speed is boosted by one stage. If this Pok\xE9mon becomes burned, it will immediately be cured of its burn status and its speed will be boosted by one stage.",
  ...
  failsafe: {
    desc: "The Pok\xE9mon heals itself by 1/8 when the move it uses fails. [Stomping Tantrum Trigger]",
  ...
  angelsguidance: {
    ...
    onFoeDamagingHit(damage, target, source, move) {
      if (source.ability == "Angel's Guidance" || source.ability == "angelsguidance")
        var angel = true;
      else
        var angel = false;
      if (!angel)
      ...
      if (possibleTypes.includes(source.types[0]) || possibleTypes.includes(source.types[1]))
        return;
  ...
  irradiation: {
    desc: "If the Pok\xE9mon successfully lands an attack, opposing Pok\xE9mon will have their Defense and Special Defense lowered by one at the end of each of the next two turns. This effect does not stack with itself.",
  ...
  avicebron: {
    desc: "The Pok\xE9mon receives an additional 1.5x boost in base power to physical STAB moves but always moves last in the turn (priority equivalent to Dragon Tail/Teleport).",
    shortDesc: "Physical STAB 1.5x boost but always moves last.",
    onBasePower(basePower, pokemon, target, move) {
      if (move.type == pokemon.types[0] || move.type == pokemon.types[1] || pokemon.types[3]) {
  ...
  rkssystem20: {
    desc: "When the Pok\xE9mon uses a move that is not Multi-Attack (activating before the move takes effect), Multi-Attack's type changes to match the type of the move used. If the Pok\xE9mon uses Multi-Attack, its type changes to Multi-Attack's current type at the end of the turn. The Pok\xE9mon's Defense and Special Defense increase by 1.5x when it is the same type as Multi-Attack.",
  ...
  kamikaze: {
    desc: "When this Pok\xE9mon's health drops to 1/16th or lower, it will immediately attempt to use Explosion. This ability will not activate if the opposing Pok\xE9mon is a Ghost type or has the Damp ability.",
    ...
    onUpdate(target) {
      if (target.hp <= target.maxhp / 16 && !target.kamikaze) {
        for (const enemy of target.side.foe.active) {
          if (!target || !this.isAdjacent(enemy, target))
            continue;
  ...
  inertia: {
    desc: "This Pok\xE9mon takes up to 50% reduced damage from direct attacks, based on how much faster it is than the attacker. Damage Reduced = 25 \xD7 User's Current Speed / Target's Current Speed.",
  ...
  deusexmachina: {
    shortDesc: "Raises the Pok\xE9mon's most proficient stat when its HP falls to half.",
  ...
  spiralpower: {
    desc: "Ghost-type moves used by this Pok\xE9mon and its allies have their type effectiveness against Normal changed to be super effective.",
    shortDesc: "This Pok\xE9mon and its allies' Ghost-type moves are super effective against Normal-types.",
    onStart(pokemon) {
  ...
  vigilante: {
    ...
    onAnyRedirectTarget(target, source, source2, move) {
      if (move.type !== "Dark" || ["firepledge", "grasspledge", "waterpledge"].includes(move.id))
        return;
  ...
  brokendlc: {
    desc: "Upon gaining this ability or entering the battle, the Pok\xE9mon changes type to the target's secondary typing, and so does the type of its Multi-Attack. If the target lacks a secondary typing, the ??? type (completely neutral offensively and defensively) is used.",
  ...
  rksoverload: {
    desc: "This Pok\xE9mon's typing is determined by its first move.",
  ...
  triggerhappy: {
    desc: "This Pok\xE9mon's self-destructiive move only deal 1/4th damage to the user.",
    shortDesc: "This Pok\xE9mon's self-destructiive move only deal 1/4th damage to the user.",
  ...
  superconductor1: {
    desc: "The damage that this Pok\xE9mon takes from contact moves is halved, but it also takes double damage from Electric-type moves. (Fluffy clone but an Electric weakness instead of a Fire weakness)",
  ...
  mugarkssystem: {
    ...
    onAfterMoveSecondary(target, source, move) {
      ...
      const randomType = this.sample(possibleTypes);
      if (target.isActive && move.effectType === "Move" && move.category !== "Status" && randomType !== "???" && !target.hasType(randomType)) {
  ...
  pounce: {
    desc: "This Pok\xE9mon is immune to all entry hazards. If it lands on any type of entry hazard, it lowers the Defense of adjacent opponents.",
  ...
  residrainhalf: {
    desc: "Every time another Pok\xE9mon is damaged indirectly, this Pok\xE9mon's HP is restored by half of the same amount.",
  ...
  boobytrap: {
    desc: "This Pok\xE9mon is immune to all entry hazards. If it lands on any type of entry hazard, it uses Tar Shot on all active enemy Pokemon.",
  ...
  wonderseal: {
    ...
    onAnyTryHit(target, source, move) {
      const pokemon = this.effectState.target;
      if (source !== pokemon && target !== pokemon)
        return;
  ...
  powerplant: {
    ...
    onAnyAfterEachBoost(boost, target, source) {
      if (!source || source === target || source !== this.effectState.target)
        return;
      let statsLowered = false;
      let i;
      for (i in boost) {
  ...
  shortcircuit: {
    desc: "When this Pok\xE9mon uses a Electric-type attack, damage is calculated using the user's Speed stat. Other effects that modify the Speed stats are used as normal, including stat stage changes.",
