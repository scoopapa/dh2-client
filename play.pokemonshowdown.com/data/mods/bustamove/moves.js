...
const Moves = {
  ...
  burningjealousy: {
    ...
    onAfterMoveSecondarySelf(pokemon, target, move2) {
      if (target?.statsRaisedThisTurn)
        this.boost({ spa: 1 }, pokemon, pokemon, move2);
    },
    secondary: null,
  ...
  ember: {
    ...
    priority: 0,
    flags: { protect: 1, mirror: 1 },
    onModifyPriority(priority, source, target, move2) {
  ...
  falsesurrender: {
    ...
    condition: {
      ...
      onBeforeMove(pokemon, target, move2) {
        ...
        this.boost({ def: -1 }, target, pokemon, move2);
      }
    },
  ...
  flowershield: {
    ...
    condition: {
      ...
      onTryHit(target, source, move2) {
        ...
        if (move2.flags["contact"] && !source.hasType("Grass")) {
          source.addVolatile("leechseed", target);
        }
        return this.NOT_FAIL;
      },
      onHit(target, source, move2) {
        if (move2.isZOrMaxPowered && move2.flags["contact"] && !source.hasType("Grass")) {
          source.addVolatile("leechseed", target);
        }
      }
    },
  ...
  grassyterrain: {
    ...
    condition: {
      ...
      onBasePower(basePower, attacker, defender, move2) {
        ...
        if (move2.type === "Grass" && attacker.isGrounded()) {
  ...
  ominouswind: {
    ...
    onAfterMoveSecondarySelf(pokemon, target, move2) {
      if (!target || target.fainted || target.hp <= 0) {
        this.boost({ atk: 1, def: 1, spa: 1, spd: 1, spe: 1 }, pokemon, pokemon, move2);
      } else {
  ...
  shadowforce: {
    ...
    condition: {
      ...
      onSourceModifyDamage(damage, source, target, move2) {
        if (move2.category === "Special" || move2.category === "Physical") {
          return this.chainModify(0.5);
        }
      }
    },
  ...
  sheercold: {
    ...
    basePower: 150,
    basePowerCallback(pokemon, target, move2) {
      return move2.basePower * pokemon.hp / pokemon.maxhp;
    },
    category: "Special",
    ...
    onHit(pokemon, source, target) {
      for (const target2 of source.side.foe.active) {
        if (target2.status === "slp") {
          this.add("-fail", target2);
          return null;
        }
        this.heal(pokemon.maxhp / 2, source) && target2.trySetStatus("slp");
      }
    },
    /* self: {
    ...
    condition: {
      ...
      onModifyType(move2) {
        if (move2.category !== "Status") {
          move2.type = "Water";
          this.debug(move2.name + "'s type changed to Water");
        }
      }
    },
    secondary: null,
  ...
  spark: {
    ...
    basePowerCallback(pokemon, target, move2) {
      if (pokemon.status && pokemon.status === "brn" && move2.category === "Physcial") {
        return move2.basePower * 2;
      }
      return move2.basePower;
    },
  ...
  steelroller: {
    ...
    accuracy: 100,
    basePower: 85,
    basePowerCallback(source, target, move2) {
  ...
  vinewhip: {
    ...
    onBasePower(basePower, source, target, move2) {
      ...
      if (!this.singleEvent("TakeItem", item, target.itemData, target, target, move2, item))
        return;
      if (item.id) {
        return this.chainModify(1.5);
      }
