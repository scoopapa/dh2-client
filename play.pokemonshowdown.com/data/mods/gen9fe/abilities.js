...
const Abilities = {
  ...
  lightdrive: {
    ...
    onAnyPrepareHit(source, target, move) {
      if (move.hasBounced || source === target)
        return;
  ...
  eczema: {
    ...
    onSourcePrepareHit(source, target, move) {
      if (target.volatiles["ability:unaware"]) {
        ...
        target.addVolatile("ability:unaware");
    },
    onEnd(pokemon) {
  ...
  aurashield: {
    ...
    onAnyBasePower(basePower, source, target, move) {
      if (target === source || move.category === "Status" || !move.secondaries)
        return;
      if (!move.auraBooster)
  ...
  faultyphoton: {
    ...
    onCriticalHit(target, source, move) {
      if (!target || target.species.id !== "ironmimic" || target.transformed || !target.runImmunity(move.type))
        return;
  ...
  burningpetals: {
    ...
    onAllyTryBoost(boost, target, source, effect) {
      if (source && target === source || !target.hasType("Fire"))
        return;
      let showMsg = false;
      let i;
      for (i in boost) {
  ...
  hourglass: {
    ...
    onTryBoost(boost, target, source, effect) {
      if (!source || target === source || !boost || effect.name === "Hourglass")
        return;
      let b;
      for (b in boost) {
  ...
  spongeofruin: {
    ...
    onAnyModifySpD(spd, target, source, move) {
      if (target.hasAbility(["Sponge of Ruin", "Beads of Ruin"]))
        return;
      const abilityHolder = this.effectState.target;
  ...
  lawnmowerofruin: {
    ...
    onAllyTryHitSide(target, source, move) {
      if (source === this.effectState.target || !target.isAlly(source) || move.type !== "Grass")
        return;
      this.boost({ atk: 1 }, this.effectState.target);
    },
    flags: { breakable: 1 },
  ...
  barbedchain: {
    ...
    onSourceDamagingHit(damage, target, source, move) {
      if (target.hasAbility("shielddust") || target.hasItem("covertcloak") || !target.hp || !this.checkMoveMakesContact(move, target, source))
        return;
  ...
  ultraface: {
    ...
    onSourceAfterFaint(length, target, source, effect) {
      ...
      if (effect && effect.effectType === "Move" && source.species.id === "eisephalon") {
        this.boost({ [(_a = this.effectState).bestStat || (_a.bestStat = source.getBestStat(true, true))]: length }, source);
    ...
    onAfterEachBoost(boost, target, source, effect) {
      ...
      for (i in boost) {
        if (boost[i] < 0) {
          ...
          if (stats.length)
            this.boost({ [this.sample(stats)]: 2 }, target, target, null, false, true);
          return;
        }
      }
    },
  ...
  swordofrejuvenation: {
    ...
    onAnyModifyDef(def, target, source, move) {
      const abilityHolder = this.effectState.target;
      if (target.hasAbility(["Sword of Ruin", "Sword of Rejuvenation"]))
        return;
  ...
  stonesofruin: {
    ...
    onAnyModifyDamage(damage, source, target, move) {
      const abilityHolder = this.effectState.target;
      if (source.hasAbility("Stones of Ruin") || target.getMoveHitData(move).typeMod <= 0)
        return;
      if (!move.ruinedAtk)
        move.ruinedAtk = abilityHolder;
      else if (move.ruinedAtk !== abilityHolder)
  ...
  magicmirror: {
    ...
    onTryBoost(boost, target, source, effect) {
      if (!source || target === source || !boost || ["Mirror Armor", "Magic Mirror"].includes(effect.name))
        return;
      let b;
      for (b in boost) {
        if (boost[b] < 0 && target.boosts[b] > -6) {
