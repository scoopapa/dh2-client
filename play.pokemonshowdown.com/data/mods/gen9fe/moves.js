...
const Moves = {
  ...
  smackdown: {
    ...
    condition: {
      noCopy: true,
      onStart(pokemon) {
        let applies = !(!(pokemon.hasType("Flying") || pokemon.hasAbility(["levitate", "airbornearmor", "aircontrol", "holygrail", "risingtension", "freeflight", "hellkite", "honeymoon", "aircontrol", "magnetize", "unidentifiedflyingobject", "anointed"])) || pokemon.hasItem("ironball") || pokemon.volatiles["ingrain"] || this.field.getPseudoWeather("gravity"));
  ...
  mistyterrain: {
    ...
    condition: {
      ...
      onSetStatus(status, target, source, effect) {
        if (!target.isGrounded() || target.isSemiInvulnerable() || ["psn", "tox"].includes(status.id) && source && source.hasAbility("miasma"))
          return;
