  ...
  */
  ...
  foolsnugget: {
    ...
    onTakeItem(item, source) {
      if (source.species.hasType("Rock"))
        return false;
      return true;
    }
  },
  cloudsail: {
    ...
    onUpdate(pokemon) {
      if (pokemon.hp <= pokemon.maxhp / 4) {
        if (!pokemon.side.sideConditions["tailwind"] && pokemon.useItem())
  ...
  leafcoat: {
    ...
    onEnd(pokemon) {
      if (pokemon.baseSpecies.types.includes("Grass"))
        return;
  ...
  baseball: {
    ...
    onSourceModifyAccuracy(accuracy, target, source, move) {
      if (typeof accuracy !== "number" || !move.flags["bullet"])
        return;
  ...
  itembox: {
    ...
    onStart(pokemon) {
      ...
      for (const pkmn of itemless) {
        ...
        if (pkmn.setItem(item))
          this.add("-message", `${pkmn.name} gained a ${item}!`);
      }
    },
    onEnd(pokemon) {
      this.add("-end", pokemon, `fallen${this.effectState.fallen}`, "[silent]");
