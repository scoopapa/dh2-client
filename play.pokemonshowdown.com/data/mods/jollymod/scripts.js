...
const Scripts = {
  ...
  battle: {},
  actions: {
    getDamage(source, target2, move, suppressMessages = false) {
      ...
      if (!move.ignoreImmunity || move.ignoreImmunity !== true && !move.ignoreImmunity[move.type]) {
        if (!target2.runImmunity(move.type, !suppressMessages)) {
          return false;
        }
      }
      if (move.ohko)
        return target2.maxhp;
      if (move.damageCallback)
        return move.damageCallback.call(this.battle, source, target2);
      ...
      if (move.basePowerCallback) {
        basePower = move.basePowerCallback.call(this.battle, source, target2, move);
      }
      if (!basePower)
        return basePower === 0 ? void 0 : basePower;
      basePower = this.battle.clampIntRange(basePower, 1);
      let critMult;
      let critRatio = this.battle.runEvent("ModifyCritRatio", source, target2, move, move.critRatio || 0);
      ...
      const moveHit = target2.getMoveHitData(move);
      ...
      if (moveHit.crit) {
        moveHit.crit = this.battle.runEvent("CriticalHit", target2, null, move);
      }
      basePower = this.battle.runEvent("BasePower", source, target2, move, basePower, true);
      if (!basePower)
        return 0;
      basePower = this.battle.clampIntRange(basePower, 1);
      ...
      const attacker = move.overrideOffensivePokemon === "target" ? target2 : source;
      const defender = move.overrideDefensivePokemon === "source" ? source : target2;
      ...
      let defense = defender.calculateStat(defenseStat, defBoosts, 1, target2);
      ...
      attack = this.battle.runEvent("Modify" + statTable[attackStat], source, target2, move, attack);
      defense = this.battle.runEvent("Modify" + statTable[defenseStat], target2, source, move, defense);
      if (this.battle.gen <= 4 && ["explosion", "selfdestruct"].includes(move.id) && defenseStat === "def") {
        defense = this.battle.clampIntRange(Math.floor(defense / 2), 1);
      }
      const baseDamage = tr(tr(tr(tr(2 * level / 5 + 2) * basePower * attack) / defense) / 50);
      return this.modifyDamage(baseDamage, source, target2, move, suppressMessages);
    }
  },
  side: {
    addKarma(amount) {
    ...
    swapKarma() {
      if (this.karma === void 0)
        this.karma = 0;
      if (this.foe.karma === void 0)
        this.foe.karma = 0;
      if (this.karma === 0 && this.foe.karma === 0)
        return;
    ...
    reward() {
      ...
      if (n < 30) {
        ...
        if (pokemon.item)
          this.battle.add("-message", `But ${pokemon.name} didn't have room for one!`);
        else {
        ...
        if (!pokemon.side.pokemon.filter((ally) => ally.fainted).length)
          this.battle.add("-message", `But ${pokemon.name} didn't need one!`);
  ...
  pokemon: {
    ...
    setStatus(status, source = null, sourceEffect = null, ignoreImmunities = false) {
      ...
      if (!ignoreImmunities && status.id && !(source?.hasAbility("permafrost") && ["frz", "fsb"].includes(status.id))) {
