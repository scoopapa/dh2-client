...
const Abilities = {
  ...
  hostabsorb: {
    onModifyMove(move, attacker2) {
      ...
      for (const target of attacker2.side.foe.active) {
        if (target.hasType("Grass"))
          return;
        if (target.hasAbility("goodasgold") || target.hasAbility("Good as Gold"))
          return;
        if (target.hasAbility("magicabsorb") || target.hasAbility("Magic Absorb"))
          return;
      }
  ...
  conduction: {
    onModifyMove(move, attacker2) {
      if (attacker2.baseSpecies.baseSpecies !== "gelsius" && attacker2.baseSpecies.baseSpecies !== "Gelsius") {
        return;
      }
      if (attacker2.species.name === "Gelsius-Subzero" || attacker2.species.name === "gelsiussubzero") {
        return;
      }
      if (attacker2.species.name === "Gelsius-Hundred" || attacker2.species.name === "gelsiushundred") {
        return;
      }
      if (attacker2.hp && move.type === "Ice") {
        this.add("-message", `${attacker2.name} is beginning to rapidly cool!`);
        attacker2.formeChange("Gelsius-Subzero", this.effect, true);
        this.add("-message", `${attacker2.name} transformed!`);
      } else if (attacker2.hp && move.type === "Fire") {
        this.add("-message", `${attacker2.name} is beginning to rapidly heat up!`);
        attacker2.formeChange("Gelsius-Hundred", this.effect, true);
        this.add("-message", `${attacker2.name} transformed!`);
      }
    },
    flags: { failroleplay: 1, noreceiver: 1, noentrain: 1, notrace: 1, failskillswap: 1, cantsuppress: 1 },
  ...
  respawnpunisher: {
    onAfterMoveSecondarySelf(pokemon, target, move) {
      if (!target || target.fainted || target.hp <= 0) {
        if (pokemon.ability != "respawnpunisher") {
          return;
        }
    ...
    onPrepareHit(source, target, move) {
      for (const targ of source.side.foe.active) {
        if (!targ.activeTurns) {
          if (source.ability != "respawnpunisher") {
            return;
          }
  ...
  vent: {
    onStart(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "impsaustor" && attacker.baseSpecies.baseSpecies !== "Impsaustor") {
        return;
      }
  ...
  purifyingsalt: {
    ...
    flags: { breakable: 1 },
    // TODO verify the assumption that this can be supprsed by Mold Breaker & friends
    name: "Purifying Salt",
    rating: 2,
    num: 272
  },
