...
const Moves = {
  blueshell: {
    ...
    onModifyMove(move2, source, target) {
      ...
      if (userSide.length < targetSide.length) {
        move2.basePower = 120;
      } else {
        move2.basePower = 80;
      }
    ...
    onHit(target, source) {
      ...
      for (const targetCondition of removeTarget) {
        if (target.side.removeSideCondition(targetCondition)) {
          if (!removeTarget.includes(targetCondition))
            continue;
  ...
  impostorblade: {
    ...
    onAfterMoveSecondarySelf(pokemon, target, move2) {
      if (pokemon.species.id === "impsaustor") {
        return;
      }
  ...
  remotemine: {
    ...
    onPrepareHit(target, source, move2) {
      const callerMoveId = move2.sourceEffect || move2.id;
      const moveSlot = callerMoveId === "instruct" ? source.getMoveData(move2.id) : source.getMoveData(callerMoveId);
      if (!moveSlot) {
        return false;
      }
      if (moveSlot.pp % 2 === 1) {
  ...
  shadowscratch: {
    ...
    onBasePower(basePower, pokemon, target) {
      if (target.hp * 4 <= target.maxhp * 3) {
        return this.chainModify(1.5);
      } else if (target.hp * 2 <= target.maxhp) {
        return this.chainModify(2);
      } else if (target.hp * 4 <= target.maxhp) {
        return this.chainModify(3);
      } else if (target.hp * 10 <= target.maxhp) {
  ...
  rest: {
    ...
    onHit(target, source, move2) {
      if (!target.setStatus("slp", source, move2))
        return false;
  ...
  gigatonhammer: {
    ...
    condition: {
      ...
      onBeforeMove(pokemon, target, move2) {
        if (move2.id === "gigatonhammer") {
          this.add("cant", pokemon, "move: Gigaton Hammer", move2);
  ...
  glaiverush: {
    ...
    onAfterHit(source, target, move2) {
      if (!target.hp) {
    ...
    secondary: {
      ...
      onHit(target, source) {
        ...
        if (result === 0) {
          ...
          if (target && source.ability === "unstableclaws") {
  ...
  shedtail: {
    ...
    onTryHit(target) {
      if (target.hp <= target.maxhp / 2 || target.maxhp === 1) {
  ...
  metronome: {
    ...
    onHit(target, source, effect) {
      ...
      for (const id in Moves) {
        const move2 = Moves[id];
        if (move2.realMove)
          continue;
        if (move2.isZ || move2.isMax || move2.isNonstandard)
          continue;
        if (effect.noMetronome.includes(move2.name))
          continue;
  ...
  guardingroom: {
    ...
    condition: {
      ...
      durationCallback(source, effect) {
        ...
        if (!source.hasMove("guardingroom")) {
          return 4;
        }
        return 5;
      },
