  ...
  */
  vesselofsigma: {
    ...
    onAnyModifySpe(spe, source, target, move) {
      ...
      if (source.hasAbility("Vessel of Sigma"))
        return;
  ...
  spicycream: {
    onUpdate(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Vanillite")
        return;
  ...
  phantomchute: {
    ...
    condition: {
      ...
      onSwap(pokemon) {
        if (!pokemon.fainted)
          pokemon.side.removeSlotCondition(pokemon, "phantomchute");
      }
    },
    flags: {},
    name: "Phantom Chute",
