  ...
  runMove(moveOrMoveName, pokemon, targetLoc, sourceEffect, zMove, externalMove, maxMove, originalTarget) {
    ...
    if (moveDidSomething && !move.isExternal) {
      ...
      for (const currentPoke of this.getAllActive()) {
        if (pokemon !== currentPoke && !currentPoke.isSemiInvulnerable() && (currentPoke.hasAbility("dancer") && move.flags["dance"]) || currentPoke.hasAbility("parroting") && move.category === "Status" || currentPoke.hasAbility("toxinreplica") && (move.secondaries && move.secondaries.some((secondary) => ["psn", "tox"].includes(secondary.status)) || move.status && ["psn", "tox"].includes(move.status))) {
  ...
  pokemon: {
    ...
    isGrounded(negateImmunity = false) {
      ...
      if (!negateImmunity && this.hasType("Flying") && !("roost" in this.volatiles))
        return false;
      if (this.hasAbility(["feelnopain", "magneticwaves", "stickyfloat", "etativel", "lighthearted", "leviflame", "levitability", "feelsomepain", "aerialbreak", "floatguise", "clearlyfloating", "hoverboard", "levimetal", "levistatic", "lovelessfloat", "testcram", "floatingreach", "spiritascent", "ghoulaway"]) && !this.battle.suppressingAbility(this))
        return null;
    ...
    ignoringAbility() {
      ...
      return !!(this.battle.gen >= 5 && !this.isActive || (this.volatiles["gastroacid"] || this.volatiles["sturdymold"] || neutralizinggas && this.ability !== "neutralizinggas" && this.ability !== "lemegeton") && !this.getAbility().isPermanent);
    },
    setStatus(status, source = null, sourceEffect = null, ignoreImmunities = false) {
      ...
      if (this.battle.event) {
        source || (source = this.battle.event.source || this);
        sourceEffect || (sourceEffect = this.battle.effect);
      } else if (!source)
        source = this;
  ...
  hitStepTryImmunity(targets, pokemon, move) {
    ...
    for (const [i, target] of targets.entries()) {
      if (
        ...
        /*this.gen >= 7 && */
        move.pranksterBoosted && pokemon.hasAbility(["prankster", "notfunny", "darkhumour", "flashyjokes", "lighthearted", "creepy"]) && targets[i].side !== pokemon.side && !this.dex.getImmunity("prankster", target)
      ) {
