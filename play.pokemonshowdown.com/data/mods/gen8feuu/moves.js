...
const Moves = {
  ...
  smackdown: {
    ...
    condition: {
      noCopy: true,
      onStart(pokemon) {
        let applies = !(pokemon.hasType("Flying") || pokemon.hasAbility(["feelnopain", "magneticwaves", "stickyfloat", "etativel", "lighthearted", "leviflame", "levitability", "feelsomepain", "aerialbreak", "floatguise", "clearlyfloating", "hoverboard", "levimetal", "levistatic", "lovelessfloat", "testcram", "floatingreach", "spiritascent", "ghoulaway"]));
  ...
  hex: {
    ...
    basePowerCallback(pokemon, target, move) {
      if (target.status || target.hasAbility(["comatose", "shadowydreams"]))
  ...
  snore: {
    onTryHit(target, source) {
      if (source.status !== "slp" && !source.hasAbility(["comatose", "shadowydreams"]))
        return false;
    }
  },
  wakeupslap: {
    inherit: true,
    basePowerCallback(pokemon, target, move) {
      if (target.status === "slp" || target.hasAbility(["comatose", "shadowydreams"]))
        return move.basePower * 2;
      return move.basePower;
    },
    //do we keep this dexited?
