...
const Abilities = {
  ...
  mythicalpresence: {
    ...
    // this happened twice independently haha
    onStart(pokemon) {
      let activated = false;
  ...
  mythicswordsman: {
    shortDesc: "The Pok\xE9mon's contact moves become special.",
    onModifyMove(move) {
  ...
  gunkmissile: {
    ...
    onSourceTryPrimaryHit(target, source, effect) {
      if (effect && effect.id === "sludgewave" || effect.id === "gunkshot" && source.hasAbility("gunkmissile") && source.species.name === "Cramorant-Alola" && !source.transformed) {
  ...
  iceface: {
    ...
    onDamagePriority: 1,
    onDamage(damage, target, source, effect) {
      if (effect && effect.effectType === "Move" && effect.category === "Physical" && target.species.id === "escavaliereiscue" && !target.transformed) {
    ...
    onCriticalHit(target, type, move) {
      if (!target)
        return;
      if (move.category !== "Physical" || target.species.id !== "escavaliereiscue" || target.transformed)
        return;
  ...
  retribution: {
    ...
    onBoost(boost, target, source, effect) {
      ...
      if (showMsg && !effect.secondaries && target.species.id === "thunjust" && target.hp && !target.transformed && target.side.foe.pokemonLeft) {
  ...
  tacticschange: {
    ...
    onBeforeMove(attacker, defender, move) {
      if (attacker.species.baseSpecies !== "Aegislash" && !attacker.species.name.includes("Ancient") || attacker.transformed)
        return;
      if (move.category === "Status" && move.id !== "foragerspoise")
        return;
