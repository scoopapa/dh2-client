...
const Moves = {
    ...
    onAfterHit(target, source, move) {
      if (!move.hasSheerForce && source.hp) {
        for (const side of source.side.foeSidesWithConditions()) {
          if (!target.side.addSlotCondition(target, "clusterbomb"))
            return false;
          Object.assign(target.side.slotConditions[target.position]["clusterbomb"], {
            ...
            moveData: {
              ...
              onPrepareHit(target2, source2) {
                this.add("-anim", source2, "Earth Power", target2);
        ...
        for (const side of source.side.foeSidesWithConditions()) {
          ...
          Object.assign(target.side.slotConditions[target.position]["clusterbomb"], {
            ...
            moveData: {
              ...
              OnAfterHit(target2, pokemon, move2) {
                target2.side.removeSideCondition("clusterbomb");
              },
              effectType: "Move",
              isFutureMove: true,
              type: "Fire"
  ...
  voltsector: {
    ...
    isFutureMove: true,
    onTry(source, target) {
      if (!target.side.addSlotCondition(target, "voltsector"))
        return false;
      Object.assign(target.side.slotConditions[target.position]["voltsector"], {
        ...
        moveData: {
          ...
          onPrepareHit(target2, source2, move) {
            this.attrLastMove("[still]");
            this.add("-anim", target2, "Magnet Rise", target2);
          },
          effectType: "Move",
          isFutureMove: true,
          type: "Electric"
      ...
      if (source.species.baseSpecies === "Ausma")
        this.add("-message", `${source.illusion ? source.illusion.name : source.name} prepares her plane sectors for attack!`);
      else
        this.add("-message", `${source.illusion ? source.illusion.name : source.name} prepares an electrical ambush!`);
      return null;
    },
    condition: {
      ...
      onResidual(target) {
        ...
        if (data.source.species.baseSpecies === "Ausma")
          this.add("-message", `${data.target.illusion ? data.target.illusion.name : data.target.name} is being zapped by Ausma's plane sectors!`);
        else
          this.add("-message", `${data.target.illusion ? data.target.illusion.name : data.target.name} is being zapped!`);
      ...
      onEnd(target) {
        ...
        this.actions.trySpreadMoveHit([data.target], data.source, hitMove);
        if (data.source.species.baseSpecies === "Ausma")
          this.add("-message", `Ausma's plane sectors retracted!`);
        else
          this.add("-message", `The electric ambush ceased!`);
      }
    },
    secondary: null,
    target: "normal",
  ...
  rapidspin: {
    //for deterraven and ausma moves//
    num: 229,
    accuracy: 100,
    basePower: 50,
    category: "Physical",
