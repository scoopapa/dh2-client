...
const Moves = {
  ...
  pitfall: {
    ...
    onAfterMoveSecondarySelf(pokemon, target, move) {
      if (!target || target.fainted || target.hp <= 0)
        this.boost({ atk: 12 }, pokemon, pokemon, move);
    },
    category: "Physical",
    ...
    condition: {
      ...
      onResidual(side) {
        for (const ally of side.active) {
          if (ally.hasItem("heavydutyboots"))
            return;
  ...
  monkeybusiness: {
    ...
    flags: { snatch: 1 },
    onHit(target) {
      if (target.hp <= target.maxhp / 2 || target.boosts.spa >= 6 || target.maxhp === 1) {
        return false;
      }
  ...
  revivalblessing: {
    ...
    onHit(pokemon, source, move) {
      ...
      for (const ally of pokemon.side.pokemon) {
        if (ally !== source && (ally.hasAbility("regenerator") || ally.volatiles["substitute"] && !move.infiltrates)) {
          continue;
        }
        if (ally.cureStatus())
          success = true;
  ...
  maxguard: {
    ...
    condition: {
      ...
      onTryHit(target, source, move) {
        ...
        const blockedByMaxGuard = this.dex.moves.get(move.id).flags["protect"] || move.isZ || move.isMax || overrideBypassProtect.includes(move.id);
