...
const Abilities = {
  ...
  */
  absorber: {
    ...
    onDamagingHit(damage, target2, source, move) {
      if (target2.getMoveHitData(move).typeMod < 0)
        this.heal(target2.baseMaxhp * 0.18);
    },
    ...
    onStart(pokemon) {
      ...
      for (const target2 of pokemon.adjacentFoes()) {
        ...
        if (pokemonArray.length === 0)
          ...
          pickNum2 = this.sample(pokemonArray);
        ...
        if (targetArray.length === 0)
          ...
          pickNum4 = this.sample(targetArray);
        ...
        const targetMove1 = this.dex.moves.get(target2.moves[pickNum3]);
        ...
        target2.moveSlots[pickNum3] = realPokemonMove1;
        target2.baseMoveSlots[pickNum3] = realPokemonMove1;
        if (pickNum2 === -1 || pickNum4 === -1)
          return;
        ...
        const targetMove2 = this.dex.moves.get(target2.moves[pickNum4]);
        ...
        target2.moveSlots[pickNum4] = realPokemonMove2;
        target2.baseMoveSlots[pickNum4] = realPokemonMove2;
      }
    },
    condition: {
      duration: 1,
      ...
      onBeforeMove(pokemon, target2, move) {
        if (pokemon.moveSlots.filter((m) => m.id === move.id).length === 0) {
          ...
          this.actions.useMove(newMove, pokemon, target2);
          return null;
        }
      }
    },
  ...
  backatya: {
    onDamagingHit(damage, target2, source, move) {
      this.damage(target2.getUndynamaxedHP(damage * 2), source, target2);
    },
    flags: {},
    name: "Back at Ya!",
  ...
  badpacing: {
    ...
    condition: {
      noCopy: true,
      onStart(target2) {
        this.add("-start", target2, "ability: Bad Pacing");
  ...
  bathroombreak: {
    onAfterMove(target2, source, move) {
      if (move.type === "Water") {
        this.add("-activate", target2, "ability: Bathroom Break");
  ...
  bigstick: {
    onResidual(pokemon) {
      if (pokemon.adjacentFoes().length == 0)
        return;
      let target2 = this.sample(pokemon.adjacentFoes());
      const branchpoke = this.dex.getActiveMove("branchpoke");
      this.actions.useMove(branchpoke, pokemon, target2);
    },
    flags: {},
    name: "big stick",
  ...
  bloodsucking: {
    onStart(pokemon) {
      ...
      for (const target2 of pokemon.adjacentFoes()) {
        ...
        this.actions.useMove(leechlife, pokemon, target2);
      }
    },
    flags: {},
    name: "Bloodsucking",
  ...
  clinch: {
    ...
    onBeforeMove(source, target2, move) {
      if (move.volatileStatus === "twoturnmove") {
        delete move.volatileStatus;
        move.accuracy = true;
      }
    },
    onTryHit(source, target2) {
      target2.side.removeSideCondition("clinch");
    },
    condition: {
      duration: 1,
      onBeforeSwitchOut(pokemon) {
        ...
        if (!moveName || !moveName.flags["charge"])
          return;
  ...
  contagious: {
    onDamagingHitOrder: 1,
    onDamagingHit(damage, target2, source, move) {
      if (target2.status && !source.status && this.checkMoveMakesContact(move, source, target2, true)) {
        source.setStatus(target2.status);
        target2.cureStatus();
      }
    },
    flags: {},
  ...
  electromagneticmanipulation: {
    onUpdate(pokemon) {
      if (pokemon.adjacentFoes().length == 0)
        return;
      let target2 = this.sample(pokemon.adjacentFoes());
      if (!target2 || target2.types[0] === "Electric")
        return;
      target2.addVolatile("electromagneticmanipulation");
    },
    condition: {
      onStart(pokemon) {
      ...
      onUpdate(pokemon) {
        if (pokemon.adjacentFoes().length == 0)
          pokemon.removeVolatile("electromagneticmanipulation");
        let target2 = this.sample(pokemon.adjacentFoes());
        if (!target2 || !target2.hasAbility("electromagneticmanipulation"))
          pokemon.removeVolatile("electromagneticmanipulation");
      },
      onEnd(pokemon) {
  ...
  fumigation: {
    ...
    onDamagingHit(damage, target2, source, move) {
      ...
      this.actions.useMove(poisongas, target2, source);
    },
    flags: {},
    name: "Fumigation",
  ...
  magicmissile: {
    ...
    */
    ...
    onSourceHit(target2, source, move) {
      if (!move || !target2)
        return;
      if (target2 !== source && move.category !== "Status") {
        if (source.item || source.volatiles["gem"] || move.id === "fling")
          return;
        const yourItem = target2.takeItem(source);
        if (!yourItem)
          return;
        if (!source.setItem(yourItem)) {
          target2.item = yourItem.id;
          return;
        }
        this.add("-item", source, yourItem, "[from] ability: Magic Missile", "[of] " + target2);
      }
    },
    onDamagingHit(damage, target2, source, move) {
      if (target2.isSemiInvulnerable())
        return;
      if (target2.ignoringItem())
        return false;
      const item = target2.getItem();
      if (!this.singleEvent("TakeItem", item, target2.itemData, target2, target2, move, item))
        return false;
      if (item.id && !item.megaStone) {
        this.damage(source.baseMaxhp / 4, source, target2);
        target2.addVolatile("fling");
        if (item.is) {
          ...
          if (item.fling && item.fling.status) {
            source.trySetStatus(item.fling.status, target2);
          } else if (item.fling && item.fling.volatileStatus) {
            source.addVolatile(item.fling.volatileStatus, target2);
          }
        }
      }
    }
    ...
    onAfterMove(pokemon, source, move) {
      if (move.category === "Status") {
        if (pokemon.adjacentFoes().length == 0)
          return;
        let target2 = this.sample(pokemon.adjacentFoes());
        this.boost({ spd: -1 }, target2, pokemon, null, true);
      }
    },
    flags: {},
    name: "Mind Bloom",
  ...
  nocturnal: {
    onTryHit(target2, source, move) {
      if (target2 !== source && move.type === "Dark") {
        if (!this.heal(target2.baseMaxhp / 4)) {
          this.add("-immune", target2, "[from] ability: Nocturnal");
        }
        return null;
      }
    },
  ...
  outclass: {
    onSourceHit(target2, source, move) {
      if (!move || !target2 || target2.hasItem("terashard"))
        return;
      let targetType = target2.types[0];
      let sourceSecondaryType = "???";
      if (source.types[1])
        sourceSecondaryType = source.types[1];
      if (target2 !== source && move.category !== "Status" && !source.hasType(targetType) && targetType !== "???" && !(source.volatiles["outclass"] && !source.side.removeFishingTokens(1))) {
        source.setType([source.types[0]]);
        if (source.addType(targetType)) {
          target2.setType(target2.getTypes(true).map((type) => type === targetType ? "???" : type));
          this.add("-start", target2, "typechange", target2.types.join("/"));
          ...
          if (sourceSecondaryType !== "???")
            source.setType([source.types[0], sourceSecondaryType]);
        }
      }
    },
    condition: {},
  ...
  peckingorder: {
    ...
    onStart(pokemon) {
      ...
      for (const target2 of pokemon.side.foe.active) {
        if (!target2 || !target2.isAdjacent(pokemon))
          continue;
        if (!activated) {
        ...
        if (target2.volatiles["substitute"]) {
          ...
          this.boost({ def: -1 }, target2, pokemon, null, true);
        }
      }
    }
  },
  ...
  precognition: {
    onBeforeTurn(pokemon) {
      if (pokemon.adjacentFoes().length == 0)
        return;
      let target2 = this.sample(pokemon.adjacentFoes());
      const targetAction = this.queue.willMove(target2);
      if (!targetAction)
        return;
  ...
  preparation: {
    onAfterMove(source, target2, move) {
  ...
  puppetmaster: {
    ...
    onAnyPrepareHit(source, target2, move) {
      ...
      if (!move || move.name !== "Substitute" || move.isZ || move.isMax || move.sourceEffect === "puppetmaster")
        return;
    ...
    onDamagingHitOrder: 1,
    onDamagingHit(damage, target2, source, move) {
      if (this.checkMoveMakesContact(move, source, target2, true) && this.randomChance(1, 1e3)) {
  ...
  statleeching: {
    onFoeAfterBoost(boost, target2, source, effect) {
      if (effect?.name === "Opportunist" || effect?.name === "Stat Leeching" || effect?.name === "Mirror Herb")
        return;
      ...
      if (Object.keys(boosts).length < 1)
        return;
  ...
  superrod: {
    onAfterMove(target2, source, move) {
      if (move.type === "Water" && target2.side.fishingTokens > 0) {
        this.heal(target2.baseMaxhp / 16 * target2.side.fishingTokens);
      }
    },
    name: "Super Rod",
