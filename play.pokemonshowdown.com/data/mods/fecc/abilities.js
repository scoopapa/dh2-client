  ...
  bongcloud: {
    onStart(pokemon) {
      if (pokemon.side.totalFainted) {
        if (!pokemon.originalLevel)
          pokemon.originalLevel = pokemon.set.level;
  ...
  gorillaofruin: {
    ...
    onUpdate(pokemon) {
      for (const target of this.getAllActive()) {
        if (!target.hasAbility("gorillaofruin") && !target.volatiles["gorillaofruin"])
          target.addVolatile("gorillaofruin");
      }
    },
    onEnd() {
      for (const target of this.getAllActive()) {
        if (target.volatiles["gorillaofruin"])
          target.removeVolatile("gorillaofruin");
      }
    },
    condition: {
      onStart(pokemon) {
