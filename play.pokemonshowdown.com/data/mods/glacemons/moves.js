...
const Moves = {
    ...
    onEffectiveness(typeMod, target, type, move) {
      ...
      switch (move.hit) {
        case 1:
          ...
          if (!this.dex.getImmunity("Ice", target))
            immunity = -0.5;
          break;
        case 2:
          hitEffectiveness = this.dex.getEffectiveness("Fire", type);
          if (!this.dex.getImmunity("Fire", target))
            immunity = -0.5;
          break;
        case 3:
          hitEffectiveness = this.dex.getEffectiveness("Electric", type);
          if (!this.dex.getImmunity("Electric", target))
            immunity = -0.5;
          break;
      }
  ...
  fling: {
    ...
    onHit(target, source) {
      const item = source.getItem();
      if (item.id === "cursedbranch" && target.addType("Grass"))
        this.add("-start", target, "typeadd", "Grass", "[from] item: Cursed Branch");
    },
    secondary: null,
    target: "normal",
    type: "Dark",
  ...
  stealthrock: {
    ...
    condition: {
      ...
      onEntryHazard(pokemon) {
        if (pokemon.hasItem("heavydutyboots") || pokemon.hasItem("silverpowder") && pokemon.hasType("Bug"))
          return;
  ...
  spikes: {
    ...
    condition: {
      ...
      onEntryHazard(pokemon) {
        if (!pokemon.isGrounded() || pokemon.hasItem("heavydutyboots") || pokemon.hasItem("silverpowder") && pokemon.hasType("Bug"))
          return;
