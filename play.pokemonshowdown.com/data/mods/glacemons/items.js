...
const Items = {
  ...
  pikaniumz: {
    ...
    onModifyType(move, pokemon) {
      ...
      if (pokemon.baseSpecies.baseSpecies === "Pikachu" && pokemon.baseSpecies.forme === "Alola" && move.type === "Normal" && !noModifyType.includes(move.id) && !(move.isZ && move.category !== "Status") && !(move.name === "Tera Blast" && pokemon.terastallized)) {
        move.type = "Psychic";
  ...
  pikashuniumz: {
    ...
    onModifyType(move, pokemon) {
      ...
      if (pokemon.baseSpecies.forme === "Pikachu-Alola" && move.type === "Normal" && !noModifyType.includes(move.id) && !(move.isZ && move.category !== "Status") && !(move.name === "Tera Blast" && pokemon.terastallized)) {
        move.type = "Psychic";
        move.typeChangerBoosted = this.effect;
      }
    },
  ...
  friedrice: {
    ...
    onModifySpe(spe, pokemon) {
      if (pokemon.baseSpecies.baseSpecies === "Clawitzer")
        return this.chainModify(1.5);
    },
  ...
  greniniumz: {
    ...
    onAfterMove(pokemon, target, move) {
      if (move.id === "bondslicingshuriken")
        pokemon.formeChange("Greninja-Ash", "[from] item: Greninium Z", true);
    },
    num: -21,
    gen: 9,
  ...
  fossilizeddino: {
    ...
    onModifySpePriority: 5,
    onModifySpe(spe, pokemon) {
      if (pokemon.baseSpecies.baseSpecies === "Arctozolt" || pokemon.baseSpecies.baseSpecies === "Arctovish")
        return this.chainModify(1.5);
    },
    basePowerCallback(pokemon, target, move) {
  ...
  fossilizeddrake: {
    ...
    onModifySpePriority: 5,
    onModifySpe(spe, pokemon) {
      if (pokemon.baseSpecies.baseSpecies === "Dracozolt" || pokemon.baseSpecies.baseSpecies === "Dracovish")
        return this.chainModify(1.5);
    },
    basePowerCallback(pokemon, target, move) {
  ...
  polkadotbow: {
    ...
    onModifyType(move, pokemon, target) {
      ...
      if (move.type === "Normal" && !noModifyType.includes(move.id) && !(move.isZ && move.category !== "Status") && !(move.name === "Tera Blast" && pokemon.terastallized) && !(move.name === "Tera Blast" && pokemon.hasItem("legendplate"))) {
        if (move.id === pokemon.moveSlots[0].id)
          type = pokemon.types[0];
        else if (move.id === pokemon.moveSlots[1].id && types.length == 2)
  ...
  dungeonslooplet: {
    ...
    onDamage(damage, target, source, effect) {
      if (target.species.abilities["0"].id && target.species.abilities["1"].id && target.species.abilities["H"].id && target.species.abilities["S"].id !== "magicguard")
        return;
      if (effect.effectType !== "Move") {
