...
const Scripts = {
  ...
  side: {
    ...
    noChange() {
      return this.miss === this.pmiss && this.effect === this.peffect && this.crit === this.pcrit && this.status === this.pstatus;
    }
  },
  actions: {
      ...
      for (hit = 1; hit <= targetHits; hit++) {
        ...
        if (target && move.multiaccuracy && hit > 1) {
          ...
          if (!move.alwaysHit) {
            ...
            if (accuracies[accuracies.length - 1] && accuracy / 100 !== accuracies[accuracies.length - 1])
              change = true;
            ...
            if (targetHits > 3) {
              ...
              if (!change) {
                ...
                this.battle.add("-message", `(${suffix} miss: ${roundNum(product)})`);
            } else {
    ...
    secondaries(targets, source, move, moveData, isSelf) {
      ...
      for (const target of targets) {
        ...
        for (const secondary of secondaries) {
          if (!secondary.self && (!target || !target.hp))
            continue;
          if (secondary.status) {
            if (target.status)
              continue;
            if (!target.runStatusImmunity(secondary.status))
              continue;
          }
          if (secondary.volatileStatus && target.volatiles[secondary.volatileStatus])
            continue;
          if (secondary.volatileStatus === "flinch" && (!this.battle.queue.willMove(target) || target.newlySwitched))
            continue;
