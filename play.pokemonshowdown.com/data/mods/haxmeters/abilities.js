...
const Abilities = {
  ...
  */
  ...
  cutecharm: {
    inherit: true,
    onDamagingHit(damage, target, source, move) {
      if (!source.runStatusImmunity("attract") || source.volatiles["attract"])
        return;
  ...
  poisontouch: {
    ...
    onSourceDamagingHit(damage, target, source, move) {
      if (!target.hp || target.hasAbility("shielddust") || target.hasItem("covertcloak"))
        return;
  ...
  toxicchain: {
    inherit: true,
    onSourceDamagingHit(damage, target, source, move) {
      if (!target.hp || target.hasAbility("shielddust") || target.hasItem("covertcloak") || target.status || !target.runStatusImmunity("tox"))
        return;
      this.add("-message", `(${source.name}'s Ability: 30)`);
      source.side.addEffect(30);
      if (source.side.effect >= 100) {
