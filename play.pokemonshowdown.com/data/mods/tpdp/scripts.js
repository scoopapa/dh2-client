...
const Scripts = {
  ...
  pokemon: {
    ...
    runImmunity(type, message) {
      ...
      const notImmune = type === "Earths" ? this.isGrounded(negateImmunity) : negateImmunity || this.battle.dex.getImmunity(type, this);
      if (notImmune)
        return true;
      if (!message)
        return false;
    ...
    isGrounded(negateImmunity = false) {
      ...
      if (!negateImmunity && this.hasType("Wind") && !(this.hasType("???") && "perch" in this.volatiles))
        return false;
      if (this.hasAbility("aircushion"))
        return this.battle.field.isWeather("duststorm");
