...
const Abilities = {
  ...
  almsgiver: {
    ...
    condition: {
      onSwap(target) {
        target.side.removeSlotCondition(target, "almsgiver");
        if (!target.fainted) {
  ...
  bingchilling: {
    num: 1011,
    name: "Bing Chilling",
    desc: "This Pok\xE9mon's Typless moves become Winter-type and gain a 1.2x. power boost. Existing Winter-type moves also get the power boost.",
    ...
    onBasePower(basePower, pokemon, target, move) {
      if (move.bingChillingBoosted || move.type === "Winter")
  ...
  blessed: {
    ...
    onStart(pokemon) {
      if (pokemon.hasType("Serenity"))
        return false;
      if (!pokemon.addType("Serenity"))
        return false;
  ...
  blossom: {
    ...
    onSourceHit(target, source, move) {
      if (!move || !target || move.type !== "Spring")
        return;
  ...
  burningrage: {
    ...
    onBasePower(basePower, attacker, defender, move) {
      if (attacker.status === "brn") {
        if (attacker.storedStats["atk"] >= attacker.storedStats["spa"] && move.category === "Physical" || attacker.storedStats["atk"] < attacker.storedStats["spa"] && move.category === "Special") {
  ...
  conversion: {
    ...
    onBasePowerPriority: 23,
    onBasePower(basePower, pokemon, target, move) {
      if (move.conversionBoosted || move.type === "Winter")
        return this.chainModify([4915, 4096]);
    }
  },
  // Coded
  ...
  courageous: {
    ...
    onSetStatus(status, target, source, effect) {
      if (status.id !== "fer" && status.id !== "fer")
        return;
      if (effect?.status) {
  ...
  decay: {
    ...
    onSourceHit(target, source, move) {
      if (!move || !target || move.type !== "Autumn")
        return;
      if (target !== source && move.category !== "Status") {
  ...
  fluffyfloat: {
    ...
    onModifyDef(pokemon) {
      if (this.field.isTerrain("cottonfield"))
        return this.chainModify(1.5);
    },
    onModifySpe(pokemon) {
      if (this.field.isTerrain("cottonfield"))
  ...
  infinitescaling: {
    ...
    desc: "The damage of this Pok\xE9mon increases infinitely. (Turn 1: 0.8x, Turn 2: 0.9x, Turn 3: 1x, Turn 4: 1.1x, etc.) Resets upon switching out.",
  ...
  jacko: {
    ...
    onModifyDamage(damage, source, target, move) {
      if (this.field.isTerrain("pumpkinfield"))
  ...
  linger: {
    ...
    onSourceHit(target, source, move) {
      if (!move || !target || move.type !== "Winter")
        return;
      if (target !== source && move.category !== "Status") {
  ...
  magimorph: {
    ...
    onBeforeMove(attacker, defender, move) {
      if (attacker.species.baseSpecies !== "Salamoon" || attacker.transformed)
        return;
  ...
  prudentplow: {
    ...
    onPrepareHit(source, target, move) {
      if (move.selfdestruct || move.multihit)
        return;
      if (["beachball", "boulder"].includes(move.id))
        return;
  ...
  reaper: {
    ...
    onModifyMove(move) {
      if (move.drain || move.type !== "Autumn")
        return;
      move.drain = [1, 2];
    }
  },
  ...
  sacrificer: {
    ...
    condition: {
      onSwap(target) {
        target.side.removeSlotCondition(target, "sacrificer");
        if (!target.fainted) {
  ...
  shine: {
    ...
    onSourceHit(target, source, move) {
      if (!move || !target || move.type !== "Summer")
        return;
      if (target !== source && move.category !== "Status") {
