...
const Items = {
    ...
    onDisableMove(pokemon) {
      for (const moveSlot of pokemon.moveSlots) {
        ...
        if (move.onModifyType)
          move.onModifyType(move, pokemon);
        if (ability.onModifyType)
          ability.onModifyType(move, pokemon);
  ...
  pokemonstandard: {
    ...
    num: 1011,
    desc: "This Pok\xE9mon's STAB moves are boosted by 25%.",
    onModifyDamage(damage, source, target, move) {
  ...
  trickyhourglass: {
    // mostly coded in the field conditions themselves
