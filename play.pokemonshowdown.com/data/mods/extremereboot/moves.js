...
const Moves = {
  ...
  aridabsorption: {
    ...
    self: {
      onHit(pokemon2, source, move) {
        this.heal(source.baseMaxhp / 3, source, pokemon2);
      }
    },
  ...
  astralascent: {
    ...
    beforeTurnCallback(pokemon2) {
      pokemon2.addVolatile("endure");
      pokemon2.addVolatile("magiccoat");
    },
    onHit(target, source) {
      let b2;
      const negBoosts = {};
      let didBoost = false;
      for (b2 in pokemon.boosts) {
        if (pokemon.boosts[b2] < 0)
          negBoosts[b2] = pokemon.boosts[b2] * -1;
        didBoost = true;
      }
      if (didBoost)
        this.boost(negBoosts, pokemon);
  ...
  beachball: {
    ...
    basePowerCallback(pokemon2, target, move) {
      ...
      if (pokemon2.volatiles["beachball"] && pokemon2.volatiles["beachball"].hitCount) {
        bp *= Math.pow(2, pokemon2.volatiles["beachball"].hitCount);
      }
      if (pokemon2.status !== "slp")
        pokemon2.addVolatile("beachball");
      return bp;
    },
    category: "Physical",
    pp: 10,
  ...
  blasphemy: {
    ...
    condition: {
      ...
      onResidualOrder: 20,
      onResidual(pokemon2) {
        const duration = pokemon2.volatiles["blasphemy"].duration;
  ...
  blizzard: {
    ...
    onEffectiveness(typeMod, target, type2, move) {
      return typeMod + this.dex.getEffectiveness("Winter", type2);
    },
    priority: 0,
    flags: { protect: 1, mirror: 1 },
    target: "normal",
  ...
  boulder: {
    ...
    basePowerCallback(pokemon2, target, move) {
      let bp = move.basePower;
      if (pokemon2.volatiles["boulder"] && pokemon2.volatiles["boulder"].hitCount) {
        bp *= Math.pow(2, pokemon2.volatiles["boulder"].hitCount);
      }
      if (pokemon2.status !== "slp")
        pokemon2.addVolatile("boulder");
  ...
  bramblewhip: {
    ...
    condition: {
      ...
      onStart(pokemon2, source) {
        this.add("-activate", pokemon2, "move: " + this.effectState.sourceEffect, "[of] " + source);
      },
      onResidualOrder: 11,
      onResidual(pokemon2) {
        ...
        if (source && (!source.isActive || source.hp <= 0 || !source.activeTurns)) {
          delete pokemon2.volatiles["partiallytrapped"];
          this.add("-end", pokemon2, this.effectState.sourceEffect, "[partiallytrapped]", "[silent]");
          return;
        }
        this.damage(pokemon2.baseMaxhp / boundDivisor);
      },
      onEnd(pokemon2) {
        this.add("-end", pokemon2, this.effectState.sourceEffect, "[partiallytrapped]");
      },
  ...
  chaosmoon: {
    ...
    onEffectiveness(typeMod, target, type2, move) {
      return typeMod + this.dex.getEffectiveness("Storm", type2);
    },
    priority: 0,
    flags: { protect: 1, mirror: 1 },
    target: "normal",
    ...
    onHit(target, source, move) {
      ...
      for (b in pokemon.boosts) {
        if (pokemon.boosts[b] < 0)
          negBoosts[b] = pokemon.boosts[b] * -1;
        didBoost = true;
      }
      if (didBoost)
        this.boost(negBoosts, pokemon);
  ...
  cottonfield: {
    ...
    condition: {
      duration: 0,
      onSwitchIn(pokemon2) {
        if (pokemon2.hasType("Winter") || pokemon2.hasType("Storm"))
          return;
        if (pokemon2.hasAbility("fluffyfloat"))
          return;
        this.boost({ evasion: -2 }, pokemon2, this.effectState.source, this.dex.getActiveMove("cottonfield"));
        this.damage(pokemon2.maxhp / 16);
      },
    ...
    condition: {
      ...
      onTryPrimaryHit(target, source, move) {
        ...
        if (damage > target.volatiles["decoy"].hp) {
          damage = target.volatiles["decoy"].hp;
        }
  ...
  drillpiercer: {
    ...
    onEffectiveness(typeMod, target, type2) {
      if (type2 === "Sky")
        return 1;
    },
    priority: 0,
  ...
  dystopiareversal: {
    ...
    onTryMove(pokemon2, target, move) {
      if (target.hasType("Manmade"))
        return;
      this.add("-fail", pokemon2, "move: Dystopia Reversal");
      this.attrLastMove("[still]");
      return null;
    },
    onHit(pokemon2) {
      pokemon2.setType(pokemon2.getTypes(true).map((type2) => type2 === "Manmade" ? "???" : type2));
      this.add("-start", pokemon2, "typechange", pokemon2.types.join("/"), "[from] move: Dystopia Reversal");
      pokemon2.trySetStatus("psn");
    },
    priority: 0,
    flags: { protect: 1, mirror: 1, reflectable: 1 },
    target: "normal",
  ...
  eeriebargain: {
    ...
    condition: {
      ...
      onResidualOrder: 11,
      onEnd(pokemon2) {
        this.add("-end", pokemon2, this.effectState.sourceEffect, "[eeriebargain]");
      },
      onTrapPokemon(pokemon2) {
        pokemon2.tryTrap();
      }
  ...
  energyfield: {
    ...
    onPrepareHit(pokemon2, source, move) {
      let usedLastTurn = false;
      if (pokemon2.lastMove.id !== "energyfield") {
        usedLastTurn = true;
        this.damage(pokemon2.baseMaxhp / 16, pokemon2, pokemon2);
      }
      return !!this.queue.willAct() && this.runEvent("StallMove", pokemon2) && usedLastTurn;
    },
    onHit(pokemon2) {
      pokemon2.addVolatile("stall");
    },
  ...
  energysiphon: {
    ...
    condition: {
      ...
      onResidualOrder: 8,
      onResidual(pokemon2) {
        const target = this.effectState.source.side.active[pokemon2.volatiles["energysiphon"].sourcePosition];
        ...
        if (damage) {
          this.heal(damage / 2, target, pokemon2);
        }
      }
    }
  },
  ...
  enlighten: {
    ...
    onHit(target, source) {
      target.cureStatus();
      if (!target.volatiles["enlighten"])
        target.addVolatile("enlighten");
    },
    condition: {
      onStart(pokemon2) {
        this.add("-start", pokemon2, "move: Enlighten");
      },
      onResidualOrder: 7,
      onResidual(pokemon2) {
  ...
  enrage: {
    ...
    condition: {
      duration: 5,
      onStart(pokemon2) {
        this.add("-singlemove", pokemon2, "Enrage");
      },
      onHit(target, source, move) {
    ...
    condition: {
      ...
      onEnd(target) {
        if (target && !target.fainted) {
          ...
          if (damage)
            this.add("-heal", target, target.getHealth, "[from] move: Floral Breeze", "[wisher] " + this.effectState.source.name);
        }
      }
    },
    secondary: null,
  ...
  hibernate: {
    ...
    flags: { snatch: 1, heal: 1 },
    onTryMove(pokemon2) {
      if (pokemon2.hp === pokemon2.maxhp) {
        this.add("-fail", pokemon2, "heal");
        return null;
      }
      if (pokemon2.status === "slp" || pokemon2.hasType("Night")) {
  ...
  hitodama: {
    ...
    priority: 0,
    onHit(target, source) {
      if (source.hasType("Folklore"))
        target.addVolatile("temporarytrap", source);
    },
  ...
  ironbash: {
    ...
    onAfterMoveSecondarySelf(pokemon2, target, move) {
      if (!target || target.fainted || target.hp <= 0)
        this.boost({ def: 1 }, pokemon2, pokemon2, move);
    },
    target: "normal",
    secondary: null
  },
  ...
  meditation: {
    ...
    onHit(target, source) {
      if (target.status && (target.status === "crs" || target.status === "fer"))
  ...
  mindcleansing: {
    ...
    condition: {
      ...
      onStart(pokemon2) {
        this.add("-endability", pokemon2);
        this.singleEvent("End", pokemon2.getAbility(), pokemon2.abilityData, pokemon2, pokemon2, "mindcleansing");
      },
      onCopy(pokemon2) {
        if (pokemon2.getAbility().isPermanent)
          pokemon2.removeVolatile("mindcleansing");
      }
    },
    priority: 0,
  ...
  moonblade: {
    ...
    condition: {
      ...
      onStart(pokemon2) {
        this.add("-endability", pokemon2);
        this.singleEvent("End", pokemon2.getAbility(), pokemon2.abilityData, pokemon2, pokemon2, "moonblade");
      },
      onCopy(pokemon2) {
        if (pokemon2.getAbility().isPermanent)
          pokemon2.removeVolatile("moonblade");
      }
    },
    priority: 0,
    flags: { protect: 1, mirror: 1, contact: 1 },
  ...
  nightynight: {
    ...
    pp: 15,
    type: "Night",
    shortDesc: "Causes target to get a 'Sleepy' status. Sleepy causes Pok\xE9mon with it to fall asleep after 3 turns. This move can be used against a target affected with the Sleepy status to advance the Sleepy counter by one stage. (Sound)",
    volatileStatus: "nightynight",
    condition: {
      onStart(pokemon2) {
        this.effectState.sleepy = -2;
        if (pokemon2.status === "slp")
          pokemon2.removeVolatile("nightynight");
        this.add("-start", pokemon2, "move: Nighty Night");
      },
      onResidual(pokemon2) {
        if (pokemon2.status === "slp") {
          pokemon2.removeVolatile("nightynight");
        ...
        if (this.effectState.sleepy >= 3) {
          pokemon2.trySetStatus("slp");
          this.add(-"end", pokemon2, "Nighty Night");
          pokemon2.removeVolatile("nightynight");
        } else {
          const timer = 3 - this.effectState.sleepy;
          const str = timer === 1 ? "turn!" : "turns!";
          this.add("-message", pokemon2.name + " will fall asleep in " + timer + " " + str);
        }
      },
      onDamagingHit(damage, target, source, move) {
        if (move.id === "nightynight") {
          this.effectState.sleepy++;
          if (this.effectState.sleepy > 0)
            this.add("-message", target.name + " became even more drowsy!");
        }
        if (this.effectState.sleepy >= 3) {
      ...
      onEnd(pokemon2) {
        this.add("-end", pokemon2, "move: Nighty Night");
      }
    },
    priority: 0,
    flags: { protect: 1, mirror: 1, sound: 1 },
  ...
  pheromonalgas: {
    ...
    condition: {
      ...
      onStart(pokemon2, source, effect) {
        ...
        this.add("-start", pokemon2, "Pheromonal Gas");
      },
      onUpdate(pokemon2) {
        if (this.effectState.source && !this.effectState.source.isActive && pokemon2.volatiles["pheromonalgas"]) {
          this.debug("Removing Pheromonal Gas volatile on " + pokemon2);
          pokemon2.removeVolatile("pheromonalgas");
        }
      },
      onBeforeMovePriority: 2,
      onBeforeMove(pokemon2, target, move) {
        this.add("-activate", pokemon2, "move: Pheromonal Gas", "[of] " + this.effectState.source);
        if (this.randomChance(1, 2)) {
          this.add("cant", pokemon2, "Pheromonal Gas");
          return false;
        }
      },
      onEnd(pokemon2) {
        this.add("-end", pokemon2, "Pheromonal Gas", "[silent]");
      }
    },
    priority: 0,
    target: "normal",
  ...
  pollinate: {
    ...
    condition: {
      duration: 3,
      onStart(pokemon2) {
        this.add("-start", pokemon2, "move: Pollinate");
      },
      onResidualOrder: 7,
      onResidual(pokemon2) {
      ...
      onEnd(pokemon2) {
        this.damage(pokemon2.baseMaxhp / 12);
        this.add("-end", pokemon2, "move: Pollinate");
      }
    },
    priority: 1,
    flags: { protect: 1, mirror: 1 },
  ...
  powerword: {
    ...
    onTryHit(target, source) {
      if (target.hp / target.baseMaxhp > 0.25)
        return false;
    },
    onHit(target, source) {
      if (target.hp / target.baseMaxhp <= 0.25)
  ...
  pricklybush: {
    ...
    onTryHitSide() {
      if (!this.field.isTerrain("rosefield"))
        return false;
    },
    condition: {
      duration: 5,
  ...
  pumpkinfield: {
    ...
    condition: {
      duration: 0,
      onSwitchIn(pokemon2) {
        if (pokemon2.hasType("Folklore") || pokemon2.hasType("Night"))
          return;
        if (pokemon2.hasAbility("jacko"))
          return;
        this.damage(pokemon2.maxhp / 16);
      },
      onSetStatus(status, target, source, effect) {
      ...
      onResidual(field) {
        for (const side of field.battle.sides) {
          for (const pokemon2 of side.active) {
            if (!pokemon2.m.lastField || pokemon2.m.lastField !== "pumpkinfield") {
              pokemon2.m.lastField = "pumpkinfield";
              pokemon2.m.fieldTurns = 0;
            }
            if (pokemon2.hasAbility("jacko"))
              continue;
            pokemon2.m.fieldTurns++;
            if (pokemon2.m.fieldTurns > pokemon2.activeTurns)
              pokemon2.m.fieldTurns = pokemon2.activeTurns;
            if (pokemon2.m.fieldTurns === 3) {
              pokemon2.trySetStatus("crs", pokemon2.side.foe.active[0], this.field.getTerrain());
            }
          }
        }
      },
  ...
  purification: {
    ...
    onPrepareHit(target, source, move) {
      if (move.hasBounced || !source.hasType("Storm"))
        return;
  ...
  rabidmaw: {
    ...
    condition: {
      duration: 3,
      durationCallback(target, source, effect) {
        if (target.status && target.status === "psn")
          return 6;
        return 3;
      },
      onStart(pokemon2) {
        this.add("-start", pokemon2, "move: Rabid Maw");
        this.add("-message", pokemon2.name + " can't heal!");
      },
      onDisableMove(pokemon2) {
        for (const moveSlot of pokemon2.moveSlots) {
      ...
      onBeforeMove(pokemon2, target, move) {
        if (move.flags["heal"] && !move.isZ && !move.isMax) {
          this.add("cant", pokemon2, "move: Rabid Maw", move);
          return false;
        }
      },
      onResidualOrder: 17,
      onEnd(pokemon2) {
        this.add("-end", pokemon2, "move: Rabid Maw");
      },
  ...
  repair: {
    ...
    condition: {
      onStart(pokemon2) {
        this.add("-message", pokemon2.name + "went under repair!");
      },
      onResidualOrder: 7,
      onResidual(pokemon2) {
        this.heal(pokemon2.baseMaxhp / 16);
      ...
      onDragOut(pokemon2) {
        this.add("-activate", pokemon2, "move: Repair");
        return null;
      }
    },
    target: "self",
  ...
  restart: {
    ...
    onHit(pokemon2) {
      ...
      const negBoosts = {};
      let didBoost = false;
      for (b2 in pokemon2.boosts) {
        if (pokemon2.boosts[b2] < 0)
          negBoosts[b2] = pokemon2.boosts[b2] * -1;
        didBoost = true;
      }
      if (didBoost)
        this.boost(negBoosts, pokemon2);
      return pokemon2.cureStatus() || success;
    },
    secondary: null,
    priority: 0,
  ...
  ricefield: {
    ...
    condition: {
      duration: 0,
      onSwitchIn(pokemon2) {
        if (pokemon2.hasAbility("bigbellied")) {
          if (pokemon2.hasType("Serenity") || pokemon2.hasType("Sea"))
            pokemon2.heal(pokemon2.baseMaxhp / 2);
          else
      ...
      onResidual(field) {
        for (const side of field.battle.sides) {
          for (const pokemon2 of side.active) {
            if (!pokemon2.m.lastField || pokemon2.m.lastField !== "ricefield") {
              pokemon2.m.lastField = "ricefield";
              pokemon2.m.fieldTurns = 0;
            }
            if (pokemon2.hasAbility("bigbellied"))
              continue;
            ...
            if (pokemon2.m.fieldTurns === 3) {
              ...
              if (pokemon2.hasType("Serenity") || pokemon2.hasType("Sea"))
                this.heal(pokemon2.baseMaxhp / 2, pokemon2);
              else
                this.heal(pokemon2.baseMaxhp / 4, pokemon2);
            }
          }
        }
      },
  ...
  risingsun: {
    ...
    onModifyType(move, pokemon2) {
      if (pokemon2.hp / pokemon2.baseMaxhp < 0.5) {
        move.type = "Winter";
      } else {
        move.type = "Summer";
      }
    ...
    onTry(source, target) {
      let type2 = "Summer";
      if (source.hp / source.baseMaxhp < 0.5)
        type2 = "Winter";
      if (!target.side.addSlotCondition(target, "futuremove"))
        return false;
      Object.assign(target.side.slotConditions[target.position]["futuremove"], {
    ...
    condition: {
      ...
      onSwitchIn(source) {
        for (const side of this.sides) {
          for (const pokemon2 of side.active) {
            if (pokemon2.hasAbility("subrosa"))
              this.heal(pokemon2.maxhp / 8, pokemon2);
          }
        }
        if (source.hasAbility("subrosa") || source.hasType("Spring") || source.hasType("Sky"))
          return;
        ...
        if (side.foe.sideConditions["rubbles"]) {
          ...
          if (allySideRubbles.layers > 0) {
            ...
            if (allySideRubbles.layers === 0) {
              ...
              if (allySideRubbles.layers === 1)
                str = " layer of Rubbles is left on ";
  ...
  seasonchange: {
    ...
    onModifyType(move, pokemon2) {
      ...
      move.type = seasons[(pokemon2.activeTurns - 1) % 4];
  ...
  seasonsend: {
    ...
    onTryMove(pokemon2, target, move) {
      if (pokemon2.hasType("Spring") || pokemon2.hasType("Summer") || pokemon2.hasType("Autumn") || pokemon2.hasType("Winter"))
        return;
      this.add("-fail", pokemon2, "move: Season's End");
      this.attrLastMove("[still]");
      return null;
    },
    ...
    self: {
      onHit(pokemon2, source, move) {
        pokemon2.setType(pokemon2.getTypes(true).map((type2) => type2 === move.type ? "???" : type2));
        this.add("-start", pokemon2, "typechange", pokemon2.types.join("/"), "[from] move: Season's End");
      }
    },
    priority: 0,
    flags: { protect: 1, mirror: 1 },
  ...
  sereneslice: {
    ...
    pp: 10,
    type: "Serenity",
    shortDesc: "This Pok\xE9mon heals 25% of its Max HP after a turn when it uses this move. Contact",
    onHit(target, source) {
    ...
    condition: {
      duration: 1,
      onEnd(pokemon2) {
        if (this.effectState.turn = pokemon2.battle.turn)
          this.heal(Math.ceil(pokemon2.maxhp * 0.25), pokemon2);
      }
    },
    priority: 0,
    flags: { protect: 1, mirror: 1, contact: 1 },
  ...
  // Coded
  shootingstar: {
    // condition is coded in conditions.ts
    name: "Shooting Star",
    accuracy: 90,
    basePower: 100,
    category: "Special",
  ...
  slipstream: {
    ...
    onHit(target, source, move) {
      ...
      for (const foe of source.side.foe.active) {
        if (foe.newlySwitched)
          switched = true;
      }
      if (switched)
        this.boost(move.boosts);
    },
    boosts: {
      spa: 1,
      spe: 1
  ...
  speedingstar: {
    ...
    onAfterMove(pokemon2, target, move) {
      if (!target || !target.m.speedingstar || target.m.speedingstar !== this.turn)
        this.heal(pokemon2.baseMaxhp / 4, pokemon2, pokemon2);
    },
    priority: 1,
    flags: { protect: 1, mirror: 1 },
    target: "normal",
  ...
  starfall: {
    ...
    condition: {
      duration: 5,
      onStart(pokemon2) {
        this.add("-start", pokemon2, "Star Fall");
      },
      onEnd(pokemon2) {
        const side = pokemon2.side.foe;
        for (const active of side.active) {
          this.damage(active.baseMaxhp / 8, active, pokemon2);
        }
        this.add("-end", pokemon2, "Star Fall");
      },
      onResidual(pokemon2) {
        const side = pokemon2.side.foe;
        for (const active of side.active) {
  ...
  starlightcrash: {
    ...
    beforeTurnCallback(pokemon2) {
      pokemon2.addVolatile("starlightcrash");
      pokemon2.volatiles["starlightcrash"].effectTarget = pokemon2;
    },
    condition: {
      duration: 1,
      onTryHit(target, source, move) {
  ...
  stockcrops: {
    ...
    onTryHit(pokemon2) {
      if (pokemon2.volatiles["stockcrops"] && pokemon2.volatiles["stockcrops"].layers >= 3)
        return false;
    },
    ...
    condition: {
      ...
      onEnd(target) {
        if (this.effectState.layers)
          this.add("-end", target, "stockcrops" + this.effectState.layers);
      }
    },
    priority: 0,
    flags: {},
  ...
  strikingtide: {
    ...
    onHit(target, source, move) {
      if (move.hit === 2)
        move.secondHit = target.hp;
      if (move.hit === 3)
        move.thirdHit = target.hp;
    },
    onAfterHit(target, source, move) {
  ...
  // Coded
  summerdaze: {
    // Chance of missing 3: .1%, The accuracy is 99.9% so if the move hits, you got at least 1 hit.
  ...
  sunflowerfield: {
    ...
    condition: {
      ...
      onResidual(field) {
        for (const side of field.battle.sides) {
          for (const pokemon2 of side.active) {
            if (!pokemon2.m.lastField || pokemon2.m.lastField !== "sunflowerfield") {
              pokemon2.m.lastField = "sunflowerfield";
              pokemon2.m.fieldTurns = 0;
            }
            pokemon2.m.fieldTurns++;
            if (pokemon2.m.fieldTurns > pokemon2.activeTurns)
              pokemon2.m.fieldTurns = pokemon2.activeTurns;
            if (pokemon2.m.fieldTurns === 3) {
              pokemon2.trySetStatus("brn", pokemon2.side.foe.active[0], this.field.getTerrain());
            }
          }
        }
      },
  ...
  swelter: {
    ...
    pp: 15,
    type: "Summer",
    shortDesc: "Deals damage to the Pok\xE9mon next to the target for 1/16 of their max HP. If there are no Pok\xE9mon next to the target, 10% chance to sunburn instead",
    priority: 0,
    flags: { protect: 1, mirror: 1 },
    target: "normal",
    secondary: {
  ...
  tearingtornado: {
    ...
    onModifyMove(move, pokemon2, target) {
      if (target && target.effectiveWeather === "rainyseason")
        move.accuracy = true;
    },
    onHit() {
      this.field.clearTerrain();
  ...
  technocut: {
    ...
    condition: {
      ...
      onResidualOrder: 20,
      onResidual(pokemon2) {
        const duration = pokemon2.volatiles["technocut"].duration;
        this.add("-start", pokemon2, "perish" + duration);
      }
    },
    priority: 0,
  ...
  twister: {
    ...
    condition: {
      ...
      onBeforeMove(pokemon2) {
        this.add("cant", pokemon2, "twister");
        return false;
      },
      onTrapPokemon(pokemon2) {
        pokemon2.tryTrap();
  ...
  typhoon: {
    ...
    condition: {
      onStart(pokemon2) {
        ...
        this.add("-message", pokemon2.name + " is building up a Typhoon!");
      },
      onFoeHit(target, source, move) {
        if (move.id !== "typhoon" || source !== this.effectState.source)
          return;
    ...
    onTryHit(target, source, move) {
      if (!source.hasType("Sea") && !source.hasType("Storm"))
        return false;
    },
    onModifyType(move, source) {
      const effect = source.volatiles["typhoon"];
      if (effect && effect.typhoonCounter && effect.typhoonCounter >= 3)
        move.type = "Storm";
    },
    basePowerCallback(pokemon2, target, move) {
      const effect = pokemon2.volatiles["typhoon"];
      if (effect && effect.typhoonCounter && effect.typhoonCounter >= 3)
        return 110;
    },
    priority: 0,
  ...
  vaccination: {
    ...
    onHit(target, source) {
      ...
      for (const vol of negativeVolatiles) {
        if (source.volatiles[vol])
          source.removeVolatile("vol");
      }
    },
  ...
  void: {
    ...
    condition: {
      ...
      duration: 5,
      onStart(pokemon2) {
        this.add("-start", pokemon2, "move: Void");
        this.add("-message", pokemon2.name + " is having its item and ability suppressed!");
      },
      onCopy(pokemon2) {
        if (pokemon2.getAbility().isPermanent)
          pokemon2.removeVolatile("void");
      },
      onEnd(pokemon2) {
        this.add("-end", pokemon2, "move: Void");
        this.add("-message", pokemon2.name + " is suppressed no more!");
      }
    },
    priority: 0,
    flags: {},
  ...
  whirlpool: {
    ...
    condition: {
      duration: 2,
      onStart(pokemon2) {
        this.add("-message", pokemon2.name + " was caught in a whirlpool!");
      },
      onTrapPokemon(pokemon2) {
        pokemon2.tryTrap();
      },
      ...
      onEnd(pokemon2) {
        this.add("-message", pokemon2.name + " was swept away!");
        pokemon2.forceSwitchFlag = true;
      }
    },
    priority: 0,
    flags: { protect: 1, mirror: 1 },
  ...
  // Coded
  wildpunch: {
    // coded in the on-contact abilities
    name: "Wild Punch",
    accuracy: 100,
    basePower: 85,
    category: "Physical",
  ...
  wilt: {
    ...
    onHit(target, source, move) {
      let healCount = 0;
      if (this.boost({ atk: -1 }, target, source, move))
        healCount++;
      if (this.boost({ spa: -1 }, target, source, move))
        healCount++;
      if (healCount)
        this.heal(source.maxhp / 8 * healCount, source, source, move);
    },
    priority: 0,
    ...
    condition: {
      ...
      onEnd(target) {
        if (target && !target.fainted) {
          ...
          if (damage)
            this.add("-heal", target, target.getHealth, "[from] move: Withering Breeze", "[wisher] " + this.effectState.source.name);
        }
      }
    },
    secondary: null,
