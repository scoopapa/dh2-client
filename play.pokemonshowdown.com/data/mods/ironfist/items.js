...
const Items = {
  ...
  fishhook: {
    ...
    onModifyMove(move, source, target) {
      if ((this.field.isTerrain("fishingterrain") && source.isGrounded() || target.hasType("Water") || target.baseSpecies.fish) && (target?.beingCalledBack || target?.switchFlag))
        move.accuracy = true;
    },
    onTryHit(source, target) {
      if (this.field.isTerrain("fishingterrain") && source.isGrounded() || target.hasType("Water") || target.baseSpecies.fish)
  ...
  bigbuttonbutton: {
    ...
    onDamagingHit(damage, target, source, move) {
      if (source.volatiles["bigbutton"] || source.terastallized && source.set.teraType === "Bug")
        return;
      if (target.useItem())
