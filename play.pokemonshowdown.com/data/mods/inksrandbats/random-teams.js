    ...
    for (const restrict of [true, false]) {
      ...
      while (pokemonPool.length && pokemon.length < 6) {
        ...
        if (restrict) {
          ...
          if (!isMonotype) {
            let skip = false;
            for (const typeName2 of types) {
              if (typeCount[typeName2] > 1) {
                skip = true;
                break;
              }
            }
          ...
          if (typeCount["Ice"] >= 3 && typeName === "Ice")
            continue;
          else if (typeCount["Ice"] === 0 && pokemon.length === 5)
            continue;
        ...
        for (const typeName2 of types) {
          if (typeName2 in typeCount) {
            typeCount[typeName2]++;
          } else {
            typeCount[typeName2] = 1;
          }
        }
