...
const Moves = {
  ...
  linkinglighthouselaunch: {
    ...
    onPrepareHit(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Mountain Gale", target);
      this.add("-anim", source2, "Fusion Flare", target);
      this.add("-anim", source2, "Stoked Sparksurfer", target);
    },
    isZ: "bondiumz",
    secondary: {
      chance: 50,
  ...
  bindingblade: {
    ...
    onPrepareHit(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Fire Spin", source2);
  ...
  bioticgrenade: {
    ...
    onTryMove(source2, target, move) {
      if (source2.isAlly(target) && source2.volatiles["healblock"]) {
        this.attrLastMove("[still]");
        this.add("cant", source2, "move: Heal Block", move);
        return false;
      }
    },
  ...
  nanoboost: {
    ...
    priority: 0,
    flags: {},
    onTryHit(source2, move) {
      if (!this.canSwitch(source2.side) || !move.selfSwitch) {
        source2.addVolatile("nanoboosted");
        return this.NOT_FAIL;
      }
    },
    onPrepareHit(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Acupressure", source2);
    },
  ...
  ultimateheal: {
    ...
    onPrepareHit(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Scale Shot", source2);
  ...
  madmilk: {
    ...
    condition: {
      ...
      onSideRestart(side) {
        if (side.sideConditions["madmilk"])
          return false;
      },
      onAfterMoveSecondaryPriority: -1,
      onAfterMoveSecondary(target, source2, move) {
        if (move.totalDamage && !source2.forceSwitchFlag) {
          this.heal(3 * move.totalDamage / 5, source2);
        }
      }
    },
    onPrepareHit(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Egg Bomb", target);
  ...
  dimensionalcape: {
    ...
    onPrepareHit(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Phantom Force", source2);
      this.add("-anim", source2, "Teleport", source2);
    },
  ...
  galaxiadarkness: {
    ...
    onPrepareHit(target, source2, move) {
      ...
      this.add("-anim", source2, "Black Hole Eclipse", source2);
    },
  ...
  upperdasharm: {
    ...
    onPrepareHit(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Sky Uppercut", target);
    },
    secondary: null,
    target: "normal",
    type: "Fairy",
  ...
  finalstrike: {
    ...
    onPrepareHit(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Judgment", target);
      this.add("-anim", source2, "Light of Ruin", target);
    },
    self: {
      boosts: {
        spa: -1
  ...
  frostbitebreath: {
    ...
    onPrepareHit(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Frost Breath", target);
    },
    secondary: null,
    target: "normal",
    type: "Ice",
  ...
  infecteddreams: {
    ...
    onPrepareHit(target, source2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", source2, "Infestation", target);
      this.add("-anim", source2, "Nightmare", target);
    },
    secondary: {
      chance: 30,
      status: "par"
    ...
    condition: {
      onSwap(target) {
        if (!target.fainted && (target.hp < target.maxhp || target.status)) {
          ...
          target.clearStatus();
          if (damage)
            this.add("-heal", target, target.getHealth, "[from] move: Medi-Gun", "[of] " + this.effectState.source);
      ...
      if (source2.hp) {
        ...
        if (item) {
          if (item.id != "duskstone") {
            this.add("-enditem", target, item.name, "[from] move: Mineralize", "[of] " + source2);
  ...
  orbofdiscord: {
    ...
    onHit(target, source2, move) {
      const success = target.addVolatile("healblock", source2, move);
      if (!success) {
  ...
  gmaxsteelsurge: {
    ...
    condition: {
      ...
      onEntryHazard(pokemon) {
        if (pokemon.hasItem("heavydutyboots") || pokemon.volatiles["hazardshield"])
          return;
  ...
  spikes: {
    ...
    condition: {
      ...
      onEntryHazard(pokemon) {
        if (!pokemon.isGrounded() || pokemon.hasItem("heavydutyboots") || pokemon.volatiles["hazardshield"])
          return;
  ...
  revivalblessing: {
    ...
    onTryHit(source2) {
      if (!source2.side.pokemon.filter((ally) => ally.fainted).length) {
        return false;
      }
    },
