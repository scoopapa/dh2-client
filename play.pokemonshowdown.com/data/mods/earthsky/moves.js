...
const Moves = {
  ...
  preservation: {
    ...
    onHit(pokemon) {
      ...
      const affectedStatuses = ["attract", "charge", "confusion", "curse", "disable", "doubleteam", "electrify", "encore", "flashfire", "focusenergy", "foresight", "gastroacid", "imprison", "ingrain", "laserfocus", "leechseed", "lockon", "magnetrise", "minimize", "miracleeye", "mindreader", "nightmare", "odorsleuth", "partiallytrapped", "perishsong", "powder", "powertrick", "preheat", "risingchorus", "saltcure", "shelter", "spotlight", "stasis", "strongpartialtrap", "tangledfeet", "tarshot", "taunt", "telekinesis", "throatchop", "torment", "trapped", "withering", "yawn"];
      for (const volatile of affectedStatuses) {
        ...
        if (returnVolatile) {
          if (pokemon.volatiles[volatile]) {
            ...
            if (["attract", "blocked", "meanlooked", "lockon", "mindreader", "partiallytrapped", "strongpartialtrap", "trapped"].includes(volatile)) {
  ...
  fairylock: {
    ...
    condition: {
      ...
      onHit(target, source, move2) {
        if (move2.selfSwitch && target !== source && !source.volatiles["substitute"] && !source.hasItem("shedshell") && !source.hasAbility("runaway"))
  ...
  odorsleuth: {
    inherit: true,
    onHit(target, source, move2) {
      return target.addVolatile("odorsleuth", source, move2, "sleuther");
    },
    condition: {
      onStart(pokemon) {
  ...
  refresh: {
    inherit: true,
    onHit(pokemon) {
      if (["", "frz"].includes(pokemon.status))
        return false;
      pokemon.cureStatus();
    },
    isNonstandard: null,
  ...
  spiderweb: {
    inherit: true,
    onHit(target, source, move2) {
      return target.addVolatile("trapped", source, move2);
    },
    isNonstandard: null,
  ...
  substitute: {
    ...
    condition: {
      ...
      onTryPrimaryHit(target, source, move2) {
        if (target === source || move2.flags["bypasssub"] || move2.infiltrates) {
          return;
        }
        let damage = this.actions.getDamage(source, target, move2);
        if (!damage && damage !== 0) {
          this.add("-fail", source);
          this.attrLastMove("[still]");
          return null;
        }
        damage = this.runEvent("SubDamage", target, source, move2, damage);
        if (!damage) {
          return damage;
        }
        ...
        if (move2.recoil || move2.id === "chloroblast") {
          this.damage(this.actions.calcRecoilDamage(damage, move2, source), source, target, "recoil");
        }
        if (move2.drain) {
          this.heal(Math.ceil(damage * move2.drain[0] / move2.drain[1]), source, target, "drain");
        }
        this.singleEvent("AfterSubDamage", move2, null, target, source, move2, damage);
        this.runEvent("AfterSubDamage", target, source, move2, damage);
