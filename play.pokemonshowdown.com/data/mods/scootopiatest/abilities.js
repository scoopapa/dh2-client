...
const Abilities = {
  ...
  powerconstruct: {
    onResidualOrder: 27,
    onResidual(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Flocura" || pokemon.transformed || !pokemon.hp)
        return;
      if (pokemon.species.id === "flocuranexus" || pokemon.hp > pokemon.maxhp / 2)
        return;
  ...
  shellbunker: {
    onDamage(damage, target, source, effect) {
      if (effect.effectType !== "Move" || !target.hurtThisTurn)
        return damage;
      return damage / 2;
    },
    name: "Shell Bunker",
  ...
  hydration: {
    inherit: true,
    onResidual(pokemon) {
      if (pokemon.status && (["raindance", "primordialsea"].includes(pokemon.effectiveWeather()) || this.dex.dataCache.scootopia.getWorldEffect() === "rainofdew")) {
