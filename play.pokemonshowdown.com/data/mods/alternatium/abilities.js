...
const Abilities = {
  galewings: {
    onModifyPriority(priority, pokemon, target, move) {
      if (move && (move.type === "Flying" || pokemon.species.id === "silvallyflying" && move.id === "multiattack") && pokemon.hp === pokemon.maxhp) {
        return priority + 1;
      }
    },
    name: "Gale Wings",
  ...
  powerofalchemy: {
    ...
    shortDesc: "This Pok\xE9mon copies the ability of the last fainted Pok\xE9mon.",
    rating: 0,
    num: 223
  },
  quickdraw: {
  ...
  rkssystem: {
    // Unused code but I'll keep it cause it can be useful in the future
    onStart(pokemon) {
  ...
  flowergift: {
    ...
    onModifyAtk(atk, source) {
      for (const pokemon of this.getAllActive()) {
        if (pokemon.hasAbility("aurabreak"))
          return;
      }
      if (source.species.id !== "shayminsky")
        return;
