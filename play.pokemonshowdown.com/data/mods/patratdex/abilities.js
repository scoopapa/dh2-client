...
const Abilities = {
  ...
  hammerhead: {
    ...
    onSourceTryPrimaryHit(target, source, effect) {
      if (source.species.baseSpecies !== "Ostrata" || source.species.name === "Ostrata-Hammer")
        return;
      if (effect && (effect.id === "drillrun" || effect.id === "dig") && source.hasAbility("hammerhead")) {
  ...
  hayveil: {
    ...
    onBeforeMove(attacker, defender, move) {
      if (attacker.species.baseSpecies !== "Incrownito" || attacker.transformed)
        return;
      if (move.category === "Status" && move.id !== "wheatshield")
        return;
  ...
  raingrow: {
    ...
    onUpdate(pokemon) {
      if (!pokemon.isActive || pokemon.baseSpecies.baseSpecies !== "Monsoonura" || pokemon.transformed)
        return;
