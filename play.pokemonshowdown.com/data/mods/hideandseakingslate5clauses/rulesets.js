...
(You have sketched ${moveName} ${count} times.)`);
  ...
  soupclause: {
    ...
    onValidateTeam(team) {
      ...
      for (const set of team) {
        ...
        (letters[firstLetter] || (letters[firstLetter] = [])).push(set.species);
      }
  ...
  disadvantageclause: {
    ...
    onFoeTrapPokemon(pokemon) {
      ...
      if (!pokemon.isAdjacent(source) || !pokemon.types.length || !source.types.length)
        return;
    ...
    onFoeMaybeTrapPokemon(pokemon, source) {
      source || (source = this.effectState.target);
  ...
  dayoneclause: {
    ...
    onValidateTeam(team) {
      ...
      for (const set of team) {
        if (letter === set.species.toLowerCase().charAt(0))
          return;
      }
  ...
  handicapclause: {
    ...
    onSwitchIn(pokemon) {
      ...
      const benched = "";
      if (!boostVal || pokemon.side.pokemon.length - 1 > pokemon.side.totalFainted)
        return;
  ...
  movedegradationclause: {
    // ask kirby about the healing move degradation
    effectType: "Rule",
    ...
    onBeforeMove(pokemon, target, move) {
      if (move.category !== "Status" || !move.flags["heal"] || target !== pokemon)
        pokemon.m.consecutiveHealing = 0;
    },
    onTryHeal(damage, target, source, effect) {
      if (!effect || effect.effectType !== "Move" || effect.category !== "Status" || target !== source)
        return;
  ...
  nonvolatilestatusclause: {
    // check if stuff that treats confusion as a volatile still works
    effectType: "Rule",
  ...
  recoverymovesclause: {
    ...
    onValidateTeam(team) {
      ...
      for (const set of team) {
        for (const move of set.moves) {
          if (healingMoves.includes(this.toID(move)))
            healers++;
        }
        if (healers > 1) {
          return [
  ...
  sigmamaleclause: {
    ...
    onChangeSet(set) {
      ...
      if (set.gender) {
        if (set.gender === "M")
          return [`${species.baseSpecies} is illegal due to Sigma Male Clause.`];
      }
    }
  },
  squawkclause: {
    ...
    onValidateTeam(team) {
      for (const set of team) {
        if (this.dex.species.get(set.species).types.includes("Flying"))
          return;
      }
      return [
  ...
  twobirdonestone: {
    ...
    onValidateTeam(team) {
      ...
      for (const set of team) {
        ...
        if (speciesTypes.includes("Flying"))
          birds++;
        if (speciesTypes.includes("Rock"))
          stones++;
      }
      if (birds === 2 && stones !== 1) {
        return [
