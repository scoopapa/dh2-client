...
const Conditions = {
  ...
  frz: {
    ...
    onModifyMove(move2, pokemon) {
      if (move2.flags["defrost"]) {
        this.add("-curestatus", pokemon, "frz", "[from] move: " + move2);
        pokemon.clearStatus();
      }
    },
    onAfterMoveSecondary(target, source, move2) {
      if (move2.thawsTarget) {
        target.cureStatus();
      }
    },
    onDamagingHit(damage, target, source, move2) {
      if (move2.type === "Fire" && move2.category !== "Status") {
        target.cureStatus();
      }
    }
  },
  ...
  choicelock: {
    ...
    onBeforeMove(pokemon, target, move2) {
      ...
      if (!pokemon.ignoringItem() && !pokemon.volatiles["dynamax"] && move2.id !== this.effectState.move && move2.id !== "struggle") {
        this.addMove("move", pokemon, move2.name);
        this.attrLastMove("[still]");
  ...
  futuremove: {
    ...
    onEnd(target) {
      ...
      if (target.fainted || target === data.source) {
        this.hint(`${move2.name} did not hit because the target is ${target.fainted ? "fainted" : "the user"}.`);
        return;
      }
      this.add("-end", target, "move: " + move2.name);
  ...
  primordialsea: {
    ...
    onTryMove(attacker, defender, move2) {
      if (move2.type === "Fire" && move2.category !== "Status") {
        ...
        this.add("-fail", attacker, move2, "[from] Primordial Sea");
        this.attrLastMove("[still]");
        return null;
      }
    },
  ...
  sunnyday: {
    ...
    onWeatherModifyDamage(damage, attacker, defender, move2) {
      if (move2.id === "hydrosteam" && !attacker.hasItem("utilityumbrella")) {
  ...
  desolateland: {
    ...
    onTryMovePriority: 1,
    onTryMove(attacker, defender, move2) {
      if (move2.type === "Water" && move2.category !== "Status") {
        this.debug("Desolate Land water suppress");
        this.add("-fail", attacker, move2, "[from] Desolate Land");
        this.attrLastMove("[still]");
        return null;
      }
    },
  ...
  deltastream: {
    ...
    onEffectiveness(typeMod, target, type, move2) {
      if (move2 && move2.effectType === "Move" && move2.category !== "Status" && type === "Flying" && typeMod > 0) {
  ...
  dynamax: {
    ...
    onSourceModifyDamage(damage, source, target, move2) {
      if (move2.id === "behemothbash" || move2.id === "behemothblade" || move2.id === "dynamaxcannon") {
  ...
  rolloutstorage: {
    name: "rolloutstorage",
    duration: 2,
    onBasePower(relayVar, source, target, move2) {
      let bp = Math.max(1, move2.basePower);
