...
const Abilities = {
  ...
  coldcommander: {
    ...
    onStart(pokemon) {
      if (this.field.isWeather(["hail", "snow"]) && pokemon.species.id === "eisugiridondozo" && !pokemon.transformed) {
    ...
    onWeatherChange(pokemon, source, sourceEffect) {
      ...
      if (!pokemon.hp)
        return;
      if (this.field.isWeather(["hail", "snow"]) && pokemon.species.id === "eisugiridondozo") {
        this.add("-activate", pokemon, "ability: Cold Commander");
        this.effectState.busted = false;
        pokemon.formeChange("Eisugiri", this.effect, true);
      }
  ...
  fishbond: {
    onSourceAfterFaint(length, target, source, effect) {
      ...
      if (source.species.id === "grenigiri" && source.hp && !source.transformed && source.side.foePokemonLeft()) {
        ...
        if (randAbil < 1)
          source.setAbility("unaware");
        else if (randAbil < 2)
          source.setAbility("waterveil");
        else
          source.setAbility("oblivious");
        this.boost({ atk: 2, def: 2, spa: 2, spd: 2, spe: 2 });
  ...
  yeah: {
    onStart(pokemon) {
      console.log(pokemon.side.pokemon.filter((pokemon2) => pokemon2.species.dondozo && !pokemon2.fainted));
      const dondozo = pokemon.side.pokemon.filter((pokemon2) => pokemon2.species.dondozo && !pokemon2.fainted).length;
  ...
  bozotodozo: {
    onSwitchOut(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Palagiri" || pokemon.transformed)
        return;
  ...
  falsedragon: {
    onBeforeSwitchIn(pokemon) {
      ...
      pokemon.illusion = new import_pokemon.Pokemon(set, pokemon.side);
    },
    onDamagingHit(damage, target, source, move) {
      if (target.illusion) {
  ...
  dondozoshield: {
    onStart(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Minigiri" || pokemon.transformed)
        return;
    ...
    onResidualOrder: 29,
    onResidual(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Minigiri" || pokemon.transformed || !pokemon.hp)
        return;
      if (pokemon.hp < pokemon.maxhp / 2) {
        if (!pokemon.species.dondozo) {
          pokemon.formeChange("Minigiri-Dondozo");
  ...
  gyeah: {
    onUpdate(pokemon) {
      ...
      if (pokemon.baseSpecies.baseSpecies !== "Malagiri" || !target.species.dondozo) {
