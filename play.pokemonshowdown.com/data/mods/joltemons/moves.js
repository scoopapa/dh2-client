...
const Moves = {
  ...
  */
  reconstruct: {
    ...
    beforeMoveCallback(pokemon) {
      if (pokemon.volatiles["reconstruct"])
        return true;
    },
    condition: {
      duration: 2,
  ...
  */
  ...
  bonemerang: {
    ...
    flags: { protect: 1, mirror: 1 },
    onHit(source, target) {
      if (!source.side.addSlotCondition(target, "futuremove") && target.hp && target.isActive)
        return false;
      Object.assign(source.side.slotConditions[target.position]["futuremove"], {
        ...
        moveData: {
          ...
          priority: -1,
          flags: { protect: 1 },
          onTryHit(target2, source2) {
            if (source2.fainted || !source2.isActive)
              return false;
          },
  ...
  stealthrock: {
    ...
    condition: {
      ...
      onSwitchIn(pokemon) {
        if (pokemon.hasItem("heavydutyboots") || pokemon.hasItem("coalengine"))
          return;
