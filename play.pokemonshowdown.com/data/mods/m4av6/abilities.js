...
const Abilities = {
  ...
  settle: {
    desc: "When using a given special move for the first time in at least three turns after its last boost, this Pok\xE9mon uses its Attack stat instead of Special Attack, and the move's power is doubled.",
    ...
    onBeforeMove(pokemon, target, move) {
      ...
      const turnsSinceLastBoost = lastBoostTurn !== void 0 ? this.turn - lastBoostTurn : Infinity;
    ...
    onResidual(pokemon) {
      ...
      for (const moveId in pokemon.m.settleCooldowns) {
        ...
        if (turnsUntilReady > 0) {
          const message = turnsUntilReady === 1 ? `${pokemon.name}'s ${moveName} will be available for a power-up next turn.` : `${pokemon.name}'s ${moveName} will be available for a power-up in ${turnsUntilReady} turn(s).`;
          this.add("-ability", pokemon, "Settle");
  ...
  */
  ...
  cheapheat: {
    ...
    desc: "When this Pok\xE9mon uses a damaging move, it boosts its relevant attacking stat (Attack or Sp. Atk) by 1 stage and the target's corresponding defending stat (Defense or Sp. Def) by 1 stage before damage is calculated. After the hit, both stats are lowered by 1 stage.",
    rating: 3,
    num: -67,
    onStart(pokemon) {
      pokemon.m.cheapHeatTargets = /* @__PURE__ */ new Map();
    ...
    onBeforeMove(pokemon, _target, move) {
      ...
      const defensiveStat = move.category === "Physical" ? "def" : "spd";
      const selfBoost = {};
