...
const Abilities = {
      ...
      if (move.category !== "Status") {
        ...
        for (const target2 of this.effectState.target.side.foe.active) {
          ...
          if (target2.volatiles["substitute"]) {
            ...
            this.boost({ spe: -1 }, target2, this.effectState.target, null, true);
          }
        }
      }
    },
  ...
  fightingspirit: {
    ...
    onResidualSubOrder: 4,
    onResidual(pokemon) {
      if (pokemon.activeMoveActions <= 6)
        return;
  ...
  parry: {
    ...
    onDamage(damage, target, source, effect) {
      if (effect && effect.effectType === "Move" && this.randomChance(1, 10) && effect.flags["contact"]) {
  ...
  lunchbox: {
    onAfterMoveSecondary(target, source, move) {
      for (const target2 of source.adjacentFoes()) {
