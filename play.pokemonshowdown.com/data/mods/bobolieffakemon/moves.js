...
const Moves = {
  ...
  healblock: {
    ...
    condition: {
      ...
      onTryHeal(damage, target, source, effect) {
        if (effect?.id === "zpower" || this.effectData.isZ)
          return damage;
        return false;
      }
    },
  ...
  maxguard: {
    ...
    condition: {
      ...
      onTryHit(target, source, move) {
        ...
        const blockedByMaxGuard = this.dex.getMove(move.id).flags["protect"] || move.isZ || move.isMax || overrideBypassProtect.includes(move.id);
  ...
  skillswap: {
    ...
    onTryHit(target, source) {
      ...
      if (target.volatiles["dynamax"] || target.getAbility().isPermanent || source.getAbility().isPermanent || additionalBannedAbilities.includes(target.ability) || additionalBannedAbilities.includes(source.ability)) {
        return false;
      }
    },
    onHit(target, source, move) {
      ...
      if (targetAbility.id !== sourceAbility.id) {
        ...
        target.abilityData = { id: this.toID(target.ability), target };
        if (target.side !== source.side)
          target.volatileStaleness = "external";
      }
