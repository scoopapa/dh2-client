...
const Abilities = {
  ...
  boombox: {
    onSourceDamagingHit(damage, target, source, move) {
      if (target.hasAbility("shielddust") || target.hasItem("covertcloak") || this.randomChance(7, 10))
        return;
  ...
  moltenfury: {
    ...
    onAllyTryHitSide(target, source, move) {
      if (source === this.effectState.target || !target.isAlly(source) || move.type !== "Rock")
        return;
  ...
  rocketpropulsion: {
    ...
    onAllyTryHitSide(target, source, move) {
      if (source === this.effectState.target || !target.isAlly(source) || move.type !== "Fire")
        return;
  ...
  twoleftfeet: {
    onAfterMoveSecondary(target, source, move) {
      if (!source || source === target || !target.hp || !move.totalDamage || move.hasBounced)
        return;
    ...
    //Currently like Magic Bounce, ideally should function like Dancer
    rating: 4
  },
  rollingboil: {
  ...
  benotafraid: {
    ...
    onDamage(damage, target, source, effect) {
      if (effect?.effectType === "Move" && target.species.id === "heirfriar" && !target.transformed) {
    ...
    onCriticalHit(target, source, move) {
      if (!target || target.species.id !== "heirfriar" || target.transformed || !target.runImmunity(move.type)) {
        return;
      }
    ...
    onEffectiveness(typeMod, target, type, move) {
      if (!target || move.category === "Status" || target.species.id !== "heirfriar" || target.transformed || !target.runImmunity(move.type)) {
        return;
      }
      const hitSub = target.volatiles["substitute"] && !move.flags["bypasssub"] && !(move.infiltrates && this.gen >= 6);
      if (hitSub)
  ...
  beantheredonethat: {
    ...
    shortDesc: "Currently works like Opportunist.",
    //After another Pokemon uses a status move that raises stats, this Pokemon uses the same move. "
    rating: 4
  },
  lawbreaker: {
    onModifyMove(move) {
