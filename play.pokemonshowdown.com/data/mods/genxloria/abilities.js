...
const Abilities = {
  ...
  deepsea: {
    ...
    onAnyBasePowerPriority: 20,
    onAnyBasePower(basePower, source, target, move) {
      if (target === source || move.category === "Status" || move.type !== "Water")
        return;
      if (!move.auraBooster)
        move.auraBooster = this.effectState.target;
      if (move.auraBooster !== this.effectState.target)
  ...
  solarflare: {
    ...
    onUpdate(pokemon) {
      if (!pokemon.isActive || pokemon.baseSpecies.baseSpecies !== "Hyakada" || pokemon.transformed)
        return;
  ...
  */
  ...
  surfsup: {
    shortDesc: "(Bugged) Before using a Water-type move or if Rain is active, this Pok\xE9mon changes to its Surfing form. This Pok\xE9mon's Water-type moves deal 1.3x damage.",
    onBeforeMovePriority: 0.5,
    onBeforeMove(attacker, defender, move) {
      if (attacker.species.baseSpecies !== "Tsunamey" || attacker.transformed)
        return;
  ...
  potionbrewer: {
    ...
    onAfterMove(target, source, move) {
      ...
      if (move.type === "Psychic" && item.isBerry)
        source.eatItem(true);
    },
    name: "Potion Brewer"
  },
