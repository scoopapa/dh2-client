...
const Abilities = {
  ...
  galewings: {
    ...
    onModifyPriority(priority, pokemon, target, move) {
      for (const poke of this.getAllActive()) {
        if (poke.hasAbility("neutralizinggas") && poke.side.id !== pokemon.side.id && !poke.volatiles["gastroacid"] && !poke.transformed) {
          return;
        }
      }
  ...
  waterbubble: {
    inherit: true,
    onSourceModifyAtkPriority: void 0,
    onSourceModifyAtk(atk, attacker, defender, move) {
    },
    onSourceModifySpAPriority: void 0,
