...
const Abilities = {
    ...
    onAfterMoveSecondarySelf(source, target, move) {
      if (move.id === "iceball" || move.id === "snowroller") {
        if (source.boosts["spe"] < 6) {
          ...
          if (!this.canSwitch(source.side) || source.forceSwitchFlag || source.switchFlag)
            return;
  ...
  patchnote: {
    // Datagon2 and Datagon-Z
    onSourceModifyDamage(damage, source, target, move) {
      let mod = 1;
      if (this.dex.types.get(move.type).damageTaken[target.getTypes()[0]] === 1)
        mod /= 2;
  ...
  shatteringclay: {
    ...
    shortDesc: "Rocks damage surrounding Pok\xE9mon when hit with a physical move.",
    rating: 3,
    num: -16
  },
  chainlink: {
    // Verdant Corviknight (please let this still work askdfjh)
    onUpdate(pokemon) {
      if (!pokemon.isStarted || pokemon.terastallized)
        return;
      if (!pokemon.hasType("Steel")) {
        for (const ally of pokemon.allies()) {
          if (ally.hasAbility("chainlink") || ally.terastallized)
            continue;
      ...
      onEnd(pokemon) {
        for (const ally of pokemon.allies()) {
          if (ally.hasAbility("chainlink")) {
            ...
            let types2 = ally.baseSpecies.types;
            if (ally.getTypes().join() !== types2.join() && ally.setType(types2))
  ...
  witheringglare: {
    ...
    onStart(pokemon) {
      ...
      this.add("-message", `${pokemon.name}'s intensity prevents all Pok\xE9mon's stats from being changed!`);
    },
    ...
    shortDesc: "Pok\xE9mon's stats can't be raised or lowered.",
    rating: 3,
    num: -18
  },
  calcify: {
  ...
  scaleshift: {
    // Ma'adowrian Frosmoth - please work the same
    onUpdate(pokemon) {
      let newtype = pokemon.baseSpecies.types[0];
      for (const ally of pokemon.side.active) {
        let scaletype = ally.types[0];
        if (ally.terastallized)
          scaletype = ally.teraType;
        if (ally && ally !== pokemon && !ally.fainted && !ally.hasAbility("scaleshift") && scaletype !== pokemon.baseSpecies.types[0] && scaletype !== pokemon.baseSpecies.types[1]) {
          newtype = scaletype;
          break;
        }
      }
  ...
  centrifuge: {
    ...
    onAnyRedirectTarget(target, source, source2, move) {
      if (move.type !== "Ground" || move.flags["pledgecombo"])
        return;
  ...
  cheaptricks: {
    ...
    onTryHitPriority: 1,
    onTryHit(target, source, move) {
      if (target === source || move.type !== "Dark" || move.category === "Status" || source.hasAbility("stickyhold"))
        return;
  ...
  zenmode: {
    // modded for Kalosian Qwilfish line
    inherit: true,
    onResidual(pokemon) {
