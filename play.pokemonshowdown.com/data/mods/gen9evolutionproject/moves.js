...
const Moves = {
  ...
  renewingring: {
    ...
    condition: {
      ...
      onStart(pokemon, source) {
        if (this.activeMove.id !== "renewingring" || !this.activeTarget.lastDamage)
          return false;
  ...
  blownfuse: {
    ...
    condition: {
      ...
      onStart(target) {
        ...
        this.hint(`Electric-types and non-grounded Pok\xE9mon are unaffected.`);
    ...
    type: "Electric",
    contestType: "Cool",
    shortDesc: "4 turns: grounded Pok\xE9mon in target slot burned after using physical moves, except Electric-types."
  },
  shadowbox: {
    num: -8,
    accuracy: 90,
    ...
    condition: {
      ...
      onResidual(pokemon) {
        ...
        if (damage) {
          ...
          let fakeSeed = { name: "Leech Seed", id: "leechseed" };
  ...
  dispersion: {
    ...
    onModifyType(move, pokemon) {
      if (pokemon.m.scaleshift)
        move.type = pokemon.m.scaleshift;
    },
    onPrepareHit(target, source, move) {
      this.attrLastMove("[still]");
  ...
  defog: {
    ...
    onHit(target, source, move) {
      ...
      for (const pokemon of this.getAllActive()) {
        if (this.runEvent("ClearFuse", pokemon))
          success = true;
      }
      return success;
    }
  ...
  tidyup: {
    ...
    onHit(pokemon) {
      ...
      for (const pokemon2 of this.getAllActive()) {
        if (this.runEvent("ClearFuse", pokemon2))
          success = true;
      }
      if (success)
