  ...
  */
  ...
  ligmaaura: {
    ...
    onAnyBasePower(basePower, source, target, move) {
      if (target === source || move.category === "Status" || move.type !== "Fire")
        return;
      if (!move.auraBooster?.hasAbility("Ligma Aura"))
  ...
  plunder: {
    onAfterMoveSecondarySelf(source, target, move) {
      if (source.stole || !move || !target || !target.lastMove || target.moveSlots.length === 1)
        return;
      if (target !== source && move.category !== "Status") {
        ...
        for (let i = 0; i < target.moveSlots.length; i++) {
          ...
          if (moveSlot === null || source.moveSlots.some((move2) => move2.name === temp.move || temp.move !== target.lastMove.name))
            continue;
        ...
        if (learnedMove)
          this.add("-message", `${source.name} stole ${target.name}'s ${learnedMove.move}!`);
      }
    },
    flags: {},
    name: "Plunder",
