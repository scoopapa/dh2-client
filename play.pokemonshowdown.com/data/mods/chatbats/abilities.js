...
const Abilities = {
  ...
  berserk: {
    onUpdate(pokemon) {
      ...
      if (pokemon.species.id !== "infernape" || !pokemon.hp)
        return;
  ...
  biogenesis: {
    ...
    onSwitchIn(pokemon) {
      ...
      if (!pokemon)
        return;
      const attackingMoves = pokemon.baseMoveSlots.map((slot) => this.dex.moves.get(slot.id)).filter((move) => move.category === "Physical" || move.category === "Special");
