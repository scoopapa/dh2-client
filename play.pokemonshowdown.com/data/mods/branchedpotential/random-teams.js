  ...
  randomTeam() {
    ...
    for (const restrict of [true, false]) {
      ...
      while (pokemonPool.length && pokemon.length < 6) {
        ...
        if ((species.name === "Zoroark" || species.name === "Inteleon-Mega") && pokemon.length > 4)
          continue;
        ...
        if (restrict) {
          ...
          if (isMega) {
            ...
            if (megaCount === 0 && pokemon.length === 5)
              continue;
          }
          if (modded) {
            hasModded = true;
          } else {
            if (hasModded === false && pokemon.length === 5)
              continue;
          }
        }
        ...
        if (set.ability === "Drizzle" || set.moves.includes("raindance") || species === "Vanilluxe-Mega")
          teamDetails["rain"] = 1;
        if (set.ability === "Drought" || set.moves.includes("sunnyday"))
          teamDetails["sun"] = 1;
        if (set.ability === "Sand Stream" || set.ability === "Sand Spit")
          teamDetails["sand"] = 1;
        if (set.ability === "Snow Warning" || species === "Vanilluxe-Mega")
          teamDetails["hail"] = 1;
