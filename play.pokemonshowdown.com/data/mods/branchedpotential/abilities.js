...
const Abilities = {
  shieldsdown: {
    onStart(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Minior" && pokemon.baseSpecies.baseSpecies !== "Stacragus" || pokemon.transformed)
        return;
    ...
    onResidualOrder: 27,
    onResidual(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Minior" && pokemon.baseSpecies.baseSpecies !== "Stacragus" || pokemon.transformed || !pokemon.hp)
        return;
      if (pokemon.hp > pokemon.maxhp / 2) {
        if (pokemon.baseSpecies.baseSpecies === "Minior" && pokemon.species.forme !== "Meteor") {
          pokemon.formeChange("Minior-Meteor");
    ...
    onSetStatus(status, target, source, effect) {
      if (target.species.id !== "miniormeteor" && target.species.id !== "stacraguschrysalis" || target.transformed)
        return;
      if (effect?.status) {
  ...
  esiugsid: {
    onDamage(damage, target, source, effect) {
      if (effect && effect.effectType === "Move" && ["phankyr"].includes(target.species.id) && !target.transformed) {
  ...
  potionmaster: {
    onModifyMove(move) {
      if (!move || move.category !== "Special" || move.target === "self")
        return;
      if (!move.secondaries) {
        move.secondaries = [];
      }
