...
const Moves = {
  ...
  filletaway: {
    ...
    onTry(source) {
      if (source.hp <= source.maxhp / 3 || source.maxhp === 1)
        return false;
    },
    onHit(pokemon) {
  ...
  thorntail: {
    ...
    onModifyMove(move, pokemon, target) {
      if (!target.hasType("Grass")) {
        (move.secondaries || (move.secondaries = [])).push({ chance: 10, volatileStatus: "leechseed" });
      }
    },
    secondary: null,
    target: "normal",
  ...
  lratio: {
    ...
    onHit(target, source, move) {
      if (!source.isAlly(target))
        this.hint(move.category + " L+Ratio");
    },
    onAfterSubDamage(damage, target, source, move) {
      if (!source.isAlly(target))
        this.hint(move.category + " L+Ratio");
  ...
  acidtrip: {
    ...
    onTry(source) {
      if (source.hp <= source.maxhp / 8 || source.maxhp === 1)
        return false;
    },
  ...
  smackdown: {
    ...
    condition: {
      noCopy: true,
      onStart(pokemon) {
        let applies = !(!(pokemon.hasType("Flying") || pokemon.hasAbility(["levitate", "soaringspirit"]) || pokemon.species.name === "Fulmenops-Stormy" && pokemon.hasAbility("weatherflux") || this.getAllActive().some((target) => target.hasAbility("treetopper")) && !["Diglett", "Dugtrio", "Sandygast", "Palossand"].includes(pokemon.baseSpecies.baseSpecies)) || pokemon.hasItem("ironball") || pokemon.volatiles["ingrain"] || this.field.getPseudoWeather("gravity"));
  ...
  */
  ...
  guardianwind: {
    ...
    condition: {
      ...
      onSetStatus(status, target, source, effect) {
        if (!effect || !source || effect.id === "yawn" || effect.effectType === "Move" && effect.infiltrates && target.side !== source.side)
          return;
        if (target !== source) {
