...
const Items = {
  ...
  soullink: {
    ...
    onDamage(damage, target, source, effect) {
      if (!effect || effect.id === "struggle" || effect.id === "soullink")
        return;
    ...
    onPrepareHit(target, source, move) {
      ...
      (move.secondaries || (move.secondaries = [])).push({
        ...
        onHit(target2) {
          if (!target2.hp)
            return;
          let targetmove = target2.lastMove;
          if (!targetmove || targetmove.isZ)
            return;
          if (targetmove.isMax && targetmove.baseMove)
            targetmove = this.dex.moves.get(targetmove.baseMove);
          const ppDeducted = target2.deductPP(targetmove.id, 4);
          if (ppDeducted)
            this.add("-activate", target2, "move: Spite", targetmove.name, ppDeducted);
        }
      });
    },
    condition: {
  ...
  ruggedscale: {
    ...
    onPrepareHit(target, source, move) {
      if (!move || move.id !== "fling")
        return;
      (move.secondaries || (move.secondaries = [])).push({ chance: 30, boosts: { def: -1 } });
    },
  ...
  serenescale: {
    ...
    onPrepareHit(target, source, move) {
      if (!move || move.id !== "fling")
        return;
      (move.secondaries || (move.secondaries = [])).push({ chance: 10, status: "frz" });
    },
  ...
  darlingscale: {
    ...
    onPrepareHit(target, source, move) {
      if (!move || move.id !== "fling")
        return;
      (move.secondaries || (move.secondaries = [])).push({ chance: 30, boosts: { spd: -1 } });
    },
  ...
  agedscale: {
    ...
    onPrepareHit(target, source, move) {
      if (!move || move.id !== "fling")
        return;
      (move.secondaries || (move.secondaries = [])).push({ chance: 30, volatileStatus: "flinch" });
    },
  ...
  syncstone: {
    ...
    onBasePower(basePower, pokemon, target, move) {
      if (move && !move.multihit && move.id !== "fling" && pokemon.useItem())
        return 90;
    },
    onPrepareHitPriority: 1,
    onPrepareHit(target, source, move) {
      if (!move || move.id !== "fling")
        return;
      (move.secondaries || (move.secondaries = [])).push({ chance: 100, boosts: { atk: 1, spa: 1 } });
    }
  },
  boostingbismuth: {
  ...
  lethargytablet: {
    ...
    onPrepareHit(target, source, move) {
      if (!move || move.id !== "fling")
        return;
      (move.secondaries || (move.secondaries = [])).push({ chance: 100, boosts: { spe: -2 } });
    }
  },
  platefossil: {
    name: "Plate Fossil",
    ...
    onPrepareHit(target, source, move) {
      if (!move || move.id !== "fling")
        return;
      (move.secondaries || (move.secondaries = [])).push({ chance: 100, boosts: { def: -1 } });
    },
  ...
  slushisloshiscale: {
    ...
    ignoreKlutz: true,
    onStart(pokemon) {
      if (!["Slushisloshi", "Wishiwashi"].includes(pokemon.baseSpecies.baseSpecies) || pokemon.transformed || pokemon.level < 20)
        return;
