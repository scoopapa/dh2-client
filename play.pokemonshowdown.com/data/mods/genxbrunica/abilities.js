...
const Abilities = {
  ...
  guardianofnature: {
    ...
    onTerrainChange(pokemon) {
      ...
      if (!strongTerrains.includes(terrain) && pokemon.hp && (this.field.terrainState.source !== pokemon && terrain || !(pokemon.beingCalledBack || pokemon.switchFlag)))
        this.field.setTerrain("guardianofnature");
    },
    onEnd(pokemon) {
  ...
  ripcurrent: {
    ...
    onSourceDamagingHit(damage, target, source, move) {
      if (source.hp && target.hp && !this.checkMoveMakesContact(move, source, target) && !target.forceSwitchFlag && !target.hasAbility("shielddust") && !target.hasItem("covertcloak") && this.randomChance(1, 4)) {
  ...
  thagomizer: {
    ...
    onBasePower(basePower, attacker, defender, move) {
      ...
      if (moveid.startsWith("tail") || moveid.endsWith("tail") || ["powerwhip", "firelash", "slam", "breakingswipe"].includes(moveid)) {
    ...
    onStart(pokemon) {
      ...
      for (const target of pokemon.foes()) {
        for (const moveSlot of target.moveSlots) {
          ...
          if (move.ohko)
            bp = 150;
          else if (["counter", "metalburst", "mirrorcoat"].includes(move.id))
            bp = 120;
          else if (bp === 1)
            bp = 80;
          if (bp > warnBp) {
  ...
  schooling: {
    ...
    onStart(pokemon) {
      if (!["Slushisloshi", "Wishiwashi"].includes(pokemon.baseSpecies.baseSpecies) || pokemon.level < 20 || pokemon.transformed || pokemon.hasItem("slushisloshiscale"))
        return;
      if (pokemon.hp > pokemon.maxhp / 4) {
  ...
  thoughtful: {
    ...
    onStart(pokemon) {
      ...
      for (i = pokemon.side.pokemon.length - 1; i > pokemon.position; i--) {
        if (pokemon.side.pokemon[i] && !pokemon.side.pokemon[i].fainted)
          break;
      }
      if (!pokemon.side.pokemon[i])
        return;
      let chosenTeammate = pokemon.side.pokemon[i];
      if (pokemon === chosenTeammate || [493, 773].includes(chosenTeammate.species.num))
        return;
      let newBaseTypes = chosenTeammate.getTypes().filter((type) => type !== "???");
      if (!newBaseTypes.length || !pokemon.setType(newBaseTypes))
        return;
