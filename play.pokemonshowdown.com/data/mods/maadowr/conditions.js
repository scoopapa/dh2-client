...
const Conditions = {
  ...
  flyingengravingeffect: {
    ...
    onUpdate(pokemon) {
      if (pokemon.hasType("Flying") || pokemon.baseSpecies.name === "Aegislash-Ma'adowr" && (pokemon.hasType("Grass") && pokemon.hasType("Steel") || pokemon.hasType("Grass") && pokemon.hasType("???"))) {
  ...
  grassfeature: {
    ...
    noCopy: true,
    onStart(pokemon) {
      this.effectState.faintedCount = 0;
    },
    onUpdate(pokemon) {
      ...
      if (faintedGrassTypes !== this.effectState.faintedCount) {
        this.effectState.faintedCount = faintedGrassTypes;
    ...
    onBasePower(basePower, attacker, defender, move) {
      ...
      if (this.effectState.faintedCount > 0) {
        ...
        return this.chainModify([powerBoost[boostIndex], 4096]);
      }
    }
    //	onEnd(pokemon) {
    ...
    onAfterMove(source, target, move) {
      ...
      for (const activeTarget of this.getAllActive()) {
        ...
        if (statLowered) {
          ...
          this.damage(damage, activeTarget, source);
  ...
  flyingfeature: {
    ...
    onBasePower(basePower, attacker, defender, move) {
      ...
      if (basePowerAfterMultiplier <= 60 && (attacker.hasType("Flying") || attacker.baseSpecies.name === "Aegislash-Ma'adowr" && (attacker.hasType("Grass") && attacker.hasType("Steel") || attacker.hasType("Grass") && attacker.hasType("???")))) {
  ...
  arastinithfeature: {
    ...
    onAfterMove(source, targets, move) {
      ...
      if (affectedTargets.includes(source))
        return;
      if (move && move.target === "allAdjacentFoes") {
        for (const foe of source.foes()) {
          if (foe.isAdjacent(source)) {
  ...
  craftenirfeature: {
    ...
    onUpdate(pokemon) {
      if (!pokemon.item && !pokemon.volatiles["jewellimit"]) {
        const currentType = pokemon.types[0];
        const speciesPrimaryType = pokemon.species.types[0];
        let jewel;
        const typeToUse = currentType === "???" ? speciesPrimaryType : currentType;
        switch (typeToUse) {
          case "Grass":
            jewel = "Grass Gem";
            break;
  ...
  golurkfeature: {
    ...
    //	},
    onUpdate(pokemon) {
      const hpThreshold = pokemon.maxhp / 3;
      if (pokemon.hp > hpThreshold) {
        if (pokemon.volatiles["golurklock"] && !["choicescarf", "choiceband", "choicespecs"].includes(pokemon.item)) {
  ...
  klinklangfeature: {
    ...
    onResidual(pokemon) {
      ...
      const hasPlusAllyMinus = pokemon.hasAbility("plus") && pokemon.side.active.some((ally) => ally && ally !== pokemon && (ally.hasAbility("minus") || ally.hasItem("magnetmodule") && (ally.hasType("Electric") || ally.hasType("Rock") || ally.hasType("Steel"))));
      const hasMinusAllyPlus = pokemon.hasAbility("minus") && pokemon.side.active.some((ally) => ally && ally !== pokemon && (ally.hasAbility("plus") || ally.hasItem("magnetmodule") && (ally.hasType("Electric") || ally.hasType("Rock") || ally.hasType("Steel"))));
      ...
      if (turnNumber % 2 === 0 && (hasPlusAllyMinus || hasMinusAllyPlus || hasMagnetModuleAllyPlusMinus)) {
        this.boost({ atk: 1, spa: 1 }, pokemon);
      } else if (turnNumber % 2 === 1 && (hasPlusAllyMinus || hasMinusAllyPlus || hasMagnetModuleAllyPlusMinus)) {
  ...
  lanturnfeature: {
    ...
    onSourceModifyAccuracy(accuracy, source, target) {
      if (typeof accuracy === "number") {
        return this.chainModify([1300, 1e3]);
      }
    },
  ...
  spiritombfeature: {
    ...
    onSourceHit(target, source, move) {
      if (!move || !target)
        return;
      if (target !== source && !target.volatiles["embargo"] && move.category !== "Status" && target.getMoveHitData(move).typeMod > 0 && !target.volatiles["stall"]) {
  ...
  typetracker: {
    ...
    onPrepareHit(pokemon) {
      ...
      if (!pokemon.volatiles["chainlink"]) {
        if (pokemon.baseSpecies.name === "Aegislash-Ma'adowr" && pokemon.species.name !== "Aegislash-Blade-Ma'adowr" && pokemon.hasType("Water") && !pokemon.hasType("Steel") && pokemon.volatiles["soaksteeldenial"]) {
