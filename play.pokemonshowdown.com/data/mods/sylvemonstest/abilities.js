...
const Abilities = {
  "forecast": {
    ...
    shortDesc: "If this Pok\xE9mon is holding a Weather Rock, its secondary typing becomes Water/Fire/Rock/Ice/Flying/Ghost (depending on the rock) and summon the corresponding weather upon entering the field. Under Strong Winds, this mon gains the added Flying type.",
    onStart(pokemon) {
    ...
    onUpdate(pokemon) {
      ...
      switch (this.field.effectiveWeather()) {
        case "sunnyday":
        case "desolateland":
          if (pokemon.species.speciesid !== "castformsunny")
            forme = "Castform-Sunny";
          break;
        case "raindance":
        case "primordialsea":
          if (pokemon.species.speciesid !== "castformrainy")
            forme = "Castform-Rainy";
          break;
        case "hail":
          if (pokemon.species.speciesid !== "castformsnowy")
            forme = "Castform-Snowy";
          break;
        default:
          if (pokemon.species.speciesid !== "castform")
            forme = "Castform";
          break;
      }
  ...
  timewarp: {
    // copied from hematite's version in smash mods melee, which was copied from his code from mfa!
  ...
  stamina: {
    // added when fixing the synthesis bug, just as a failsafe
    inherit: true,
    onDamagingHit(damage, target, source, effect) {
      if (source === target)
        return;
  ...
  "darkrising": {
    ...
    onStart(pokemon) {
      ...
      let bestStat = 0;
      let i;
      for (const i2 in pokemon.storedStats) {
        if (pokemon.storedStats[i2] > bestStat) {
          stat = i2;
          bestStat = pokemon.storedStats[i2];
        }
      }
  ...
  "immovable": {
    shortDesc: "This Pok\xE9mon takes 50% from all attacks when paralyzed.",
    onSourceModifyDamage(damage, source, target, move) {
      let mod = 1;
  ...
  "cursedbody": {
    shortDesc: "On switch-in, the opposing Pok\xE9mon is taunted unless it has a Substitute.",
    onStart(pokemon) {
      let activated = false;
      for (const target of pokemon.side.foe.active) {
        if (!target || !target.isAdjacent(pokemon))
  ...
  "mindtrick": {
    desc: "When this Pok\xE9mon's stat stages would be modified, other Pok\xE9mon's stat stages are modified instead. When other Pok\xE9mon's stat stages would be modified, this Pok\xE9mon's stat stages are modified instead.",
    shortDesc: "Stat changes on this Pok\xE9mon are reflected back to the attacker and vice versa.",
    onAnyBoost(boost, target, source, effect) {
      if (!boost || effect.id === "mirrorarmor" || effect.id === "mindtrick")
        return;
      if (target === this.effectState.target) {
        ...
        for (b in boost) {
          ...
          for (const pokemon of this.getAllActive()) {
            if (pokemon === this.effectState.target || pokemon.fainted)
              continue;
