...
const Abilities = {
  ...
  grimneigh: {
    onFaint(source, target) {
      for (const target2 of this.getAllActive()) {
        target2.clearBoosts();
        this.add("-clearboost", target2, "[from] ability: Grim Neigh", "[of] " + source);
        target2.cureStatus();
      }
    },
    name: "Grim Neigh",
  ...
  faultyphoton: {
    ...
    onDamage(damage, target, source, effect) {
      if (effect && effect.effectType === "Move" && target.species.id === "ironmimic" && !target.transformed) {
    ...
    onUpdate(pokemon) {
      if (pokemon.species.id === "ironmimic" && this.effectState.busted) {
        const speciesid = (
          /* pokemon.species.id === 'mimikyutotem' ? 'Mimikyu-Busted-Totem' :*/
          "Iron Mimic-Busted"
        );
