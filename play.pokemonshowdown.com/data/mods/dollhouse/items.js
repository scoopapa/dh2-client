...
const Items = {
  ...
  dragapultplushie: {
    ...
    onPrepareHit(source, target, move) {
      if (move.type === "Dragon" && !move.multihit && !move.flags["noparentalbond"] && !move.flags["charge"] && !move.flags["futuremove"] && !move.spreadHit && !move.isZ && !move.isMax && move.category !== "Status") {
        move.multihit = 2;
  ...
  ogerponhearthflameplushie: {
    ...
    onModifyType(move, pokemon) {
      ...
      if (move.type === "Grass" && !noModifyType.includes(move.id) && !(move.isZ && move.category !== "Status") && !(move.name === "Tera Blast" && pokemon.terastallized)) {
        move.type = "Fire";
      }
    },
    num: -1004,
  ...
  magearnaplushie: {
    ...
    onSourceAfterFaint(length, target, source, effect) {
      if (this.effectState.magearnaPlushie)
        return;
  ...
  garganaclplushie: {
    ...
    onSetStatus(status, target, source, effect) {
      if (target.hasType("Rock") && effect?.status) {
  ...
  chiyuplushie: {
    ...
    onAnyModifySpD(spd, target, source, move) {
      ...
      if (target.hasItem("Chi-Yu Plushie"))
        return;
      if (this.queue.willMove(target))
        return;
      if (!move.ruinedSpD?.hasItem("Chi-Yu Plushie"))
        move.ruinedSpD = itemHolder;
      if (move.ruinedSpD !== itemHolder)
        return;
