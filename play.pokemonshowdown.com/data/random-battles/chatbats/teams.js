...
class RandomTeams {
  ...
  cullMovePool(types, moves, abilities, counter, movePool, teamDetails, species, isLead, isDoubles, teraType, role) {
    ...
    if (species.id === "tatsugiri")
      this.incompatibleMoves(moves, movePool, "nastyplot", "rapidspin");
    if (species.id === "golurk")
      this.incompatibleMoves(moves, movePool, "icepunch", "dynamicpunch");
    if (species.id === "veluza")
      this.incompatibleMoves(moves, movePool, "waterfall", "hydropump");
  ...
  getPriorityItem(ability, types, moves, counter, teamDetails, species, isLead, isDoubles, teraType, role) {
    ...
    if (species.id === "golemalola")
      return "Life Orb";
    if (species.id === "ironcrown")
      return this.sample(["Chesto Berry", "Leftovers"]);
    if (species.id === "lurantis")
    ...
    if (species.id === "phione")
      return "Leftovers";
    if (species.id === "sudowoodo" && moves.has("synthesis"))
      return this.sample(["Red Card", "Leftovers"]);
    if (species.id === "sudowoodo" && moves.has("curse"))
      return this.sample(["Red Card", "Leftovers"]);
  ...
  randomTeam() {
    ...
    while (baseSpeciesPool.length && pokemon.length < this.maxTeamSize) {
      ...
      if (!this.forceMonotype && isMonotype && typeComboCount[typeCombo] >= 3 * limitFactor)
        continue;
      if (pokemon.length === 3 || this.maxTeamSize === 1)
        species = "mew";
      let set;
      if (leadsRemaining) {
