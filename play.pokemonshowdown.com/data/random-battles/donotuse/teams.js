...
class RandomTeams {
  ...
  getPriorityItem(ability, types, moves, counter, teamDetails, species, isLead, isDoubles, teraType, role) {
    ...
    if (species.id === "palkia" && counter.get("Special") < 4)
      return "Lustrous Orb";
    if (moves.has("courtchange") || !isDoubles && (species.id === "terapagos" && !moves.has("rest")))
    ...
    if (ability === "Magic Guard" && moves.has("calmmind"))
      return "Life Orb";
  ...
  getItem(ability, types, moves, counter, teamDetails, species, isLead, teraType, role) {
    ...
    if (species.id !== "jirachi" && counter.get("Physical") >= 4 && ["dragontail", "fakeout", "firstimpression", "flamecharge", "rapidspin"].every((m) => !moves.has(m))) {
      const scarfReqs = role !== "Wallbreaker" && (species.baseStats.atk >= 60 || ability === "Huge Power" || ability === "Pure Power") && species.baseStats.spe >= 40 && species.baseStats.spe <= 70 && ability !== "Speed Boost" && !counter.get("priority") && !moves.has("aquastep");
      return scarfReqs && this.randomChance(1, 2) ? "Choice Scarf" : "Choice Band";
    }
    if (counter.get("Special") >= 4 || counter.get("Special") >= 3 && ["flipturn", "uturn"].some((m) => moves.has(m))) {
      const scarfReqs = role !== "Wallbreaker" && species.baseStats.spa >= 60 && species.baseStats.spe >= 40 && species.baseStats.spe <= 70 && ability !== "Speed Boost" && ability !== "Tinted Lens" && !moves.has("uturn") && !counter.get("priority");
    ...
    if (moves.has("outrage") && counter.get("setup"))
      return "Lum Berry";
    if (moves.has("protect") && !moves.has("wish") && ability !== "Speed Boost")
      return "Leftovers";
    if (role === "Fast Support" && isLead && !counter.get("recovery") && !counter.get("recoil") && (counter.get("hazards") || counter.get("setup")) && species.baseStats.hp + species.baseStats.def + species.baseStats.spd < 140)
      return "Focus Sash";
